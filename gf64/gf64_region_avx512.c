 ...
 *     elements live in the low qword of each lane, the constant is broadcast).
 *
 ...
 *   With constant_broadcast = [c, c, c, c, c, c, c, c] and imm8=0x00:
...
void gf64_region_mul_avx512(gf64_t *HEDLEY_RESTRICT out, const gf64_t *HEDLEY_RESTRICT in, size_t len, gf64_t constant) {
	/* Broadcast the single constant once outside the loop; then do 2 VPCLMULQDQ
	 * per outer iteration (8 GF elements processed, 4 elements per call).
	 */
	__m512i constant_broadcast = _mm512_set1_epi64((int64_t)constant);

	size_t i = 0;
	size_t blocks = len / 8;
	for (size_t b = 0; b < blocks; b++) {
		/* Call 1: out[i+0..i+3] = clmul(in[i+0..i+3], constant) */
		__m512i in_lo = _mm512_set_epi64(0, (int64_t)in[i + 3], 0, (int64_t)in[i + 2],
		                                  0, (int64_t)in[i + 1], 0, (int64_t)in[i + 0]);
		__m512i prod_lo = _mm512_clmulepi64_epi128(in_lo, constant_broadcast, 0x00);
		...
		/* Call 2: out[i+4..i+7] = clmul(in[i+4..i+7], constant) */
		__m512i in_hi = _mm512_set_epi64(0, (int64_t)in[i + 7], 0, (int64_t)in[i + 6],
		                                  0, (int64_t)in[i + 5], 0, (int64_t)in[i + 4]);
		__m512i prod_hi = _mm512_clmulepi64_epi128(in_hi, constant_broadcast, 0x00);
