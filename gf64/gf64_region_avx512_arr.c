...
void gf64_region_coupled_muladd_avx512_arr(
	...
	for (size_t g = 0; g < G; g++) {
		gf64_region_muladd_avx512_arr(out, in_blocks[g], &coeff_blocks[g], len, 1);
	}
}
...
void gf64_region_fused_output_muladd_avx512_arr(
	...
	size_t K) {
	if (K == 0 || len == 0) return;
	size_t cap_k = (K > 256) ? 256 : K;
	__m512i coeff_bc[256];
	size_t active_k[256];
	size_t n_active = 0;
	for (size_t k = 0; k < cap_k; k++) {
		uint64_t c = (uint64_t)*coeff_block_starts[k];
		if (c != 0) {
			coeff_bc[n_active] = _mm512_set1_epi64((int64_t)c);
			active_k[n_active] = k;
			n_active++;
		}
	}
	...
	for (size_t b = 0; b < blocks; b++) {
		...
		for (size_t a = 0; a < n_active; a++) {
			size_t k = active_k[a];
			__m512i prod = _mm512_clmulepi64_epi128(in_vec, coeff_bc[a], 0x00);
			__m512i lo_v, hi_v;
			gf64_split_prod_512(prod, &lo_v, &hi_v);
