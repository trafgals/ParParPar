...
- **[1] 1 GiB / 1k slicing**: Coarse slicing geometry with $N=1,024 < \text{BARY\_MIN\_INPUTS\_DEFAULT}=10,000$ and $R=8 < \text{FENGER\_MIN\_R}=8,192$. Falls below the Barycentric heuristic gate at this size and routes to the SIMD matvec Cauchy kernel.
...
- **[3] 14 GiB shape (Pending measurement)**: Fenger padded geometry ($N=229{,}376$, $\text{next\_pow2}=262{,}144$). Since $R=32,768 \ge \text{FENGER\_MIN\_R}=8,192$, the Fenger (Bostan–Schost MPE) fast path engages. Kernel parity is pinned by `test/par3-fenger-padded-engine.js` (disjoint ranges, barycentric reference) and multi-chunk pipeline execution by `test/par3-chunked-inputs.js`; awaiting measured hardware throughput run.
- **[4] 16 GiB shape**: Measured on Node 22.22.3 with V8 single-Buffer cap lifted. Dispatches to the Barycentric kernel ($R=8 < \text{FENGER\_MIN\_R}$).
