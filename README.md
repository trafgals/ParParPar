...
ParParPar is a next-generation fork of [animetosho/ParPar](https://github.com/animetosho/ParPar). It adds full support for the **PAR3 archive format**, removes the slice and file size limitations of PAR2, and introduces an optimized C++ SIMD engine (AVX-512, AVX-2, and SSSE3) with streaming asynchronous I/O.
...
| **PAR3** (ParParPar) | 4 GiB Create ($R=8$) | 65,536 | 64 KiB | [![PAR3 4GB/65536 (Zen4)](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Ftrafgals%2FParParPar%2Ffeat%2Fci-benchmark-badge%2Fbenchmarks%2Fbadges%2Fpar3-4g-65536-zen4.json&style=flat-square)](https://github.com/trafgals/ParParPar#throughput) | N/A | Node 22+ auto-sized input pool <sup>[2]</sup> |
| **PAR3** (ParParPar) | 14 GiB Create ($R=32768$, pending) | 229,376 | 64 KiB | [![PAR3 14GB/229376 (Zen4)](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Ftrafgals%2FParParPar%2Ffeat%2Fci-benchmark-badge%2Fbenchmarks%2Fbadges%2Fpar3-14g-229376-zen4.json&style=flat-square)](https://github.com/trafgals/ParParPar#throughput) | N/A | Fenger padded fast path (non-pow2 input); pending measurement <sup>[3]</sup> |
| **PAR3** (ParParPar) | 16 GiB Create ($R=8$) | 262,144 | 64 KiB | [![PAR3 16GB/262144 (Zen4)](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Ftrafgals%2FParParPar%2Ffeat%2Fci-benchmark-badge%2Fbenchmarks%2Fbadges%2Fpar3-16g-262144-zen4.json&style=flat-square)](https://github.com/trafgals/ParParPar#throughput) | N/A | "Magic shape"; Node 22+; Barycentric kernel <sup>[4]</sup> |
| **PAR3** (ParParPar) | 32 GiB Create ($R=8$) | 524,288 | 64 KiB | [![PAR3 32GB/524288 (Zen4)](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Ftrafgals%2FParParPar%2Ffeat%2Fci-benchmark-badge%2Fbenchmarks%2Fbadges%2Fpar3-32g-524288-zen4.json&style=flat-square)](https://github.com/trafgals/ParParPar#throughput) | N/A | Multi-archive scale; Barycentric kernel (peak RSS 32.25 GiB, pagefile auto-grow) <sup>[5]</sup> |
| **PAR3** (ParParPar) | 64 GiB Create ($R=8$) | 1,048,576 | 64 KiB | [![PAR3 64GB/1048576 (Zen4)](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Ftrafgals%2FParParPar%2Ffeat%2Fci-benchmark-badge%2Fbenchmarks%2Fbadges%2Fpar3-64g-1048576-zen4.json&style=flat-square)](https://github.com/trafgals/ParParPar#throughput) | N/A | Memory wall; Barycentric kernel (peak RSS 48.34 GiB, swap-bound, pool exceeds RAM) <sup>[6]</sup> |
| **PAR3** (ParParPar) | 32 MiB Repair (8 loss) | 256 | 128 KiB | **4,981 ms** | — | Coupled RHS regression — see issue #90 |
| **PAR3** (ParParPar) | Matrix Solve (10k geom) | 10,000 | — | **224 ms** ($658\times$ vs JS) | — | Native SIMD matrix solver |

*All throughput measurements in MB/s (higher is better). CI column measured monthly on GitHub Actions (`ubuntu-latest`). Full benchmarking protocol and reproduction steps are documented in [BENCHMARKING.md](BENCHMARKING.md).*

#### Footnotes & Caveats

- **[1] 1 GiB / 1k slicing**: Coarse slicing geometry with $N = 1{,}024 < 10{,}000$ (`BARY_MIN_INPUTS_DEFAULT`) and $R = 8 < 8{,}192$ (`FENGER_MIN_R`). Falls below the Barycentric heuristic gate at this size and routes to the SIMD matvec Cauchy kernel.
- **[2] 4 GiB shape**: Under Node 22+ (`buffer.constants.MAX_LENGTH === Number.MAX_SAFE_INTEGER`), `poolSize = max(64 MiB, totalSize + 64 KiB)` (`lib/par3gen.js:633`) sits well within V8 limits without env-var overrides. The pool is periodically reset via `_resetInputBufferPool` (`lib/par3gen.js:622`), so memory does not accumulate across batches.
- **[3] 14 GiB shape**: Fenger padded geometry ($N=229{,}376$, `next_pow2` = 262,144). Under Node 22+ baseline, since $R = 32{,}768 \ge 8{,}192$ (`FENGER_MIN_R`), the Fenger (Bostan–Schost MPE) fast path engages. Kernel parity is pinned by `test/par3-fenger-padded-engine.js` (disjoint ranges, barycentric reference) and multi-chunk pipeline execution by `test/par3-chunked-inputs.js`; awaiting measured hardware throughput run.
- **[4] 16 GiB shape**: Measured on Node 22.22.3 with V8 single-Buffer cap lifted. Dispatches to the Barycentric kernel ($R = 8 < 8{,}192$, below `FENGER_MIN_R`).
- **[5] 32 GiB shape**: Requires ≥33 GiB system memory. Alloc pool (32 GiB + 64 KiB) and working buffers fit within physical RAM on this 63 GiB reference host; Windows pagefile auto-grew from 30 GiB to 116 GiB at peak with negligible swap pressure (peak RSS 32.25 GiB).
- **[6] 64 GiB shape (Swap-bound)**: The input pool alone (64 GiB + 64 KiB) strictly exceeds the host's 63 GiB physical RAM, making this run swap-bound. Windows pagefile auto-grew to 173 GiB at peak to fit, and the create phase thrashed the input pool through swap, accounting for ~340 s of the 371 s total create time (peak RSS 48.34 GiB). The 176 MB/s throughput reflects swap-fault overhead rather than kernel compute cost; on a host with ≥128 GiB RAM where the pool is fully resident, expect ~500 MB/s consistent with the 32 GiB baseline (552 MB/s).

#### Benchmarking Methodology & Environment

- **Reference Host (Zen4)**: AMD Ryzen 7 7800X3D (native Windows, MSVC addon, AVX-512 auto).
