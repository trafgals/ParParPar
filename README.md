...
| **PAR3** (ParParPar) | 4 GiB Create ($R=8$) | 65,536 | 64 KiB | [![PAR3 4GB/65536 (Zen4)](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Ftrafgals%2FParParPar%2Ffeat%2Fci-benchmark-badge%2Fbenchmarks%2Fbadges%2Fpar3-4g-65536-zen4.json&style=flat-square)](https://github.com/trafgals/ParParPar#throughput) | N/A | Largest pow2 shape on Node 20 — V8's 4 GiB single-Buffer cap; requires `PAR3_GF64_INPUT_POOL_SIZE=4294963200` env var to bypass the +64 KiB input-pool headroom collision (lib/par3gen.js:639 — without the env var, `poolSize = max(64 MiB, totalSize + 64 KiB)` would try `Buffer.alloc(4 GiB + 64 KiB)`, which exceeds V8's `Buffer.alloc` cap of 4 GiB exact by 65,536 B; the override sets `poolSize = 4294963200` (4 GiB - 4 KiB) which fits the cap. The pool is reset during processing via `_resetInputBufferPool` (lib/par3gen.js:622) at strategic points, so the pool doesn't need to hold the entire input at once) |
