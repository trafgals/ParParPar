...
function fileScan(files, recurse, skipSymlinks, concurrency, hashAlgo, cb) {
	...
	scanFiles = function(fileList, recDepth, visitedDirs, cbDoneScan) {
		if(typeof visitedDirs === 'function') {
			cbDoneScan = visitedDirs;
			visitedDirs = null;
		}
		if(!visitedDirs) visitedDirs = new Set();
				...
				async.waterfall([
					...
					function(stat, cbWf) {
						...
						if(stat.isDirectory()) {
							info = null;
							if(recDepth) {
								var dirId = (stat.dev !== undefined && stat.ino !== undefined && stat.ino !== 0) ? (stat.dev + ':' + stat.ino) : null;
								fs.realpath(file, function(err, realDir) {
									if(err) realDir = file;
									var identity = dirId || realDir;
									if(visitedDirs.has(identity) || visitedDirs.has(realDir)) {
										// Circular directory/symlink detected; skip to avoid infinite recursion
										if(bufHeld) { bufs.put(buf); bufHeld = false; }
										return cbWf(true);
									}
									var nextVisited = new Set(visitedDirs);
									nextVisited.add(realDir);
									if(dirId) nextVisited.add(dirId);
									...
									fs.readdir(file, function(err, dirFiles) {
										...
										}), typeof recDepth === 'number' ? recDepth - 1 : recDepth, nextVisited, function(err2, dirRes) {
					...
					function(_fd, cbWf) {
						fd = _fd;
						fs.read(fd, buf, 0, 16384, null, function(err, bytesRead, buffer) {
							if(err) {
