...
// A4 (issue #59 §8): Windows Fenger enablement is size-gated. Small-N
// Fenger is verified crash-free on native Windows (N=8192/R=512 completes;
// the old "Node 22 Windows access violation" was the large-N schoolbook
// divmod/EGCD cliff, fixed by A1). Above the gate, Windows keeps the
// scalar/Barycentric default pending native verification of the padded
// route at scale. Override with PAR3_FENGER_WINDOWS_MAX_INPUTS.
...
function dispatchRecovery(binding, inputs, outputs, numInputs, numRecovery, blockSize, firstInput, firstRecovery, numThreads, fallbackFn) {
	...
	// A4: on Windows, Fenger is additionally size-gated (small-N verified;
	// above the gate the scalar/Barycentric default stays).
	var fengerEligible = hasFenger
		&& _fengerPaddingReasonable(numInputs)
		&& _isPowerOfTwoOrTrivial(numRecovery)
		&& blockSize % 8 === 0
		&& (fengerForced || process.platform !== 'win32' || numInputs <= _fengerWindowsMaxInputs());
