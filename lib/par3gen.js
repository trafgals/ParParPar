...
// Padding-cost bound retained as a helper for future reactivation. The
// padded Fenger route (gf64_fenger_prepare_padded) is bit-exact for small
// N but uses O(N^2) tree prep + MPE, which is asymptotically slower than
// the O(N*R) legacy fallback it would replace. Subquadratic prep is a
// Phase A follow-up (issue #46); until it lands, non-power-of-2 workloads
// fall through to the legacy path and only power-of-2 shapes use Fenger.
...
function dispatchRecovery(binding, inputs, outputs, numInputs, numRecovery, blockSize, firstInput, firstRecovery, numThreads, fallbackFn) {
	...
	// non-Windows hosts. Windows (MSVC addon) keeps the scalar/Barycentric
	// default pending a native crash fix (issue #46 Phase 2 blocker —
	// Node 22 Windows access violation inside compute_recovery_fenger);
	// operators can still opt in via the env var.
	// Env contract:
	...
	// Power-of-2 gate for the subproduct tree (issue #46). A padded
	// Fenger route exists in gf64_fenger.c and is bit-exact for small N,
	// but its tree-prep + MPE are O(N^2) — slower than the O(N*R) legacy
	// fallback. Until subquadratic prep lands (Phase A follow-up), only
	// power-of-2 N/R shapes use Fenger; non-power-of-2 falls through to
	// the legacy path.
	var fengerEligible = hasFenger
		&& _isPowerOfTwoOrTrivial(numInputs)
	...
	if(fengerForced) {
		...
		if(!fengerEligible) {
			if(!_fengerFallbackWarned) {
				...
				console.warn('[par3gen] fenger-fallback-scalar: PAR3_GF64_USE_FENGER=1 but numInputs=' + numInputs + ' or numRecovery=' + numRecovery + ' is not 0/1/power-of-2; falling back to scalar kernel');
			}
		}
	}
