...
function dispatchRecovery(binding, inputs, outputs, numInputs, numRecovery, blockSize, firstInput, firstRecovery, numThreads, fallbackFn) {
	// Fenger dispatch (issue #46 K3): default for power-of-2 workloads.
	// Env contract:
	...
	if(fengerEligible && !fengerDisabled) {
		...
		process.stderr.write('[diag] compute_recovery_fenger enter\n');
		var fengerResult = binding.compute_recovery_fenger(
			inputs, outputs, numInputs, numRecovery, blockSize, firstInput, firstRecovery, numThreads
		);
		process.stderr.write('[diag] compute_recovery_fenger exit\n');
		return fengerResult;
	}
