...
run: function(eventCb, completeCb) {
	...
	global.__par3ProfStart('setup');
		...
		var envCap = parseInt(process.env.PAR3_SIMULATED_BUFFER_CAP, 10);
		var isSimulatedCap = isFinite(envCap) && envCap > 0;
		var envMemLimit = parseInt(process.env.PAR3_MEMORY_LIMIT, 10);
		var maxMemoryBudget = isFinite(envMemLimit) && envMemLimit > 0
			? envMemLimit
			: (self.opts.memoryLimit || 1024 * 1024 * 1024); // 1 GiB default budget
		var isChunkedRecovery = isSimulatedCap ? (totalInputBytes > envCap) : (totalInputBytes > maxMemoryBudget);
		...
		if(isChunkedRecovery) {
			if (isSimulatedCap) {
				chunkCapacityBlocks = Math.max(1, Math.floor(envCap / blockSize));
			} else {
				// Cubic review P1 & P2: reserve accumulator bytes (and Fenger scratch if Fenger is chosen)
				...
				chunkCapacityBlocks = Math.max(1, Math.floor(availableInputBudget / blockSize));
			}
		...
		var processFileFull = function(idx) {
			...
			try {
				...
				var bulkRead = 0;
				while(bulkRead < fileStat.size) {
					var n = fs.readSync(fd, bulkFileBuf, bulkRead, fileStat.size - bulkRead, bulkRead);
					if(n === 0) break;
					bulkRead += n;
				}
				fs.closeSync(fd);
