	...
	global.__par3ProfStart('setup');
		...
		if(useComputeRecoveryFull) {
			...
			var processFileFull = function(idx) {
				...
				// read — only when a streaming recovery is actually pending
				// (normal creates skip this syscall entirely). A source that
				// changed between the streaming read and the JS data read
				// invalidates the streaming recovery — recovery must match
				// the data packets.
				if(streamRecovery && streamStat) {
					var postStat = null;
