...
// Benchmarks across Zen4 show that 64 MiB maximizes L3 cache locality, TLB efficiency,
// and prefetch streaming while keeping peak RSS strictly < 450 MiB (~311-340 MiB measured).
// Oversized chunk caps (128/256 MiB) caused cache thrashing, a 37% throughput drop on 32G,
// and memory ballooning to 508-749 MiB due to TLB misses and virtual memory page commits.
function decideChunkCapBytes(totalInputBytes, numRecovery, blockSize, kernel, memoryLimit) {
	var chunkCapBytes = 64 * 1024 * 1024; // 64 MiB default across all archive shapes
