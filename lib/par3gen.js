...
// A4 (issue #59 §8): Windows Fenger enablement is size-gated AND
// opt-in. Small-N Fenger is verified crash-free on native Windows
// (N=8192/R=512 completes; the old "Node 22 Windows access violation"
// was the large-N schoolbook divmod/EGCD cliff, fixed by A1), but the
// default-on lift exposed a Node-20-only native crash on the
// windows-2025 CI runner at trivial shapes (N=1/R=2, silent exit 1,
// not reproducible on a local Zen4/Node 20.19 host — tracked as a
// follow-up before default-on). Windows therefore keeps the
// scalar/Barycentric default unless the operator opts in:
//   PAR3_FENGER_WINDOWS_ENABLE=1  (+ optional PAR3_FENGER_WINDOWS_MAX_INPUTS,
//   default 65536).
function _fengerWindowsMaxInputs() {
	var v = parseInt(process.env.PAR3_FENGER_WINDOWS_MAX_INPUTS, 10);
	if(!isFinite(v) || v <= 0) v = 65536;
	return v;
}

function _fengerWindowsEnabled() {
	var e = process.env.PAR3_FENGER_WINDOWS_ENABLE;
...
function dispatchRecovery(binding, inputs, outputs, numInputs, numRecovery, blockSize, firstInput, firstRecovery, numThreads, fallbackFn) {
	...
	// A4: on Windows, Fenger is additionally size-gated AND opt-in
	// (PAR3_FENGER_WINDOWS_ENABLE=1; default off pending the Node-20
	// windows-2025 native crash follow-up). Above the gate the
	// scalar/Barycentric default stays.
	var fengerEligible = hasFenger
		...
		&& (fengerForced || process.platform !== 'win32'
			|| (_fengerWindowsEnabled() && numInputs <= _fengerWindowsMaxInputs()));
