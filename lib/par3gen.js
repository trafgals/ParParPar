...
// create call. acquireInputBuffer(size) wraps inputBufferPoolOffset back to 0
// when the requested size exceeds remaining pool space (safe because block
// buffers are consumed synchronously into packets/recovery before the next batch).
// One-off Buffer.alloc is used as a fallback if size exceeds the entire pool.
...
// create entry point with totalInputBlocks * blockSize. If the pool is already
// initialized (from a prior create in the same process), this is a no-op — use
// _resetInputBufferPool to reset the offset between creates.
// Capped at 64 MiB (or PAR3_GF64_INPUT_POOL_SIZE) to break the 64 GiB memory wall.
function _ensureInputPool(totalSize) {
	if(inputBufferPool !== null) return;
	var envPoolCap = parseInt(process.env.PAR3_GF64_INPUT_POOL_SIZE, 10);
	var poolCap = (isFinite(envPoolCap) && envPoolCap > 0) ? envPoolCap : 64 * 1024 * 1024;
	var poolSize = Math.min(poolCap, Math.max(64 * 1024 * 1024, totalSize + 64 * 1024));
	poolSize = Math.min(poolSize, 0x7fffffff);
...
run: function(eventCb, completeCb) {
	...
	global.__par3ProfStart('setup');
		...
		// using compute_recovery_accumulate, eliminating the fullInputs buffer and breaking
		// the 64 GiB memory wall (RSS <= 256 MiB instead of 48+ GiB).
		...
		if (self.opts.memoryLimit && self.opts.memoryLimit > 0) {
			chunkCapBytes = Math.min(chunkCapBytes, Math.max(blockSize, Math.floor(self.opts.memoryLimit / 2)));
		}
		...
		var hasAccumulate = (routingDecision.kernel === 'fenger' && typeof bindingRef.compute_recovery_fenger === 'function') ||
			(bindingRef && typeof bindingRef.compute_recovery_accumulate === 'function');

		// cubic review 59dd8dd8 P1: Fenger kernel uses chunked recovery accumulation when totalInputBytes > chunkCapBytes
		var useChunkedCreate = !disableChunked && hasAccumulate &&
			(forceChunked || totalInputBytes > chunkCapBytes);

		if (useChunkedCreate) {
			useStreamingCreate = false;
		}
		...
		var chunkBlocks = 0;
		var chunkBlockIdx = 0;
		var chunkStartGlobalBlock = 0;
		var fengerChunkTmp = null;

		if (useChunkedCreate) {
			chunkBlocks = Math.max(1, Math.floor(chunkCapBytes / blockSize));
			chunkInputs = allocBuffer(chunkBlocks * blockSize);
			ensureAccumulator();
		} else {
			fullInputs = allocBuffer(totalInputBytes);
			// cubic review df1de4cb P2: data bytes overwritten by reads, padding zeroed per file; no full buffer zero pass
		}
		var nextBlockIdx = 0;

		function flushChunkAccumulator() {
			if(!useChunkedCreate || chunkBlockIdx === 0) return;
			ensureAccumulator();
			if (routingDecision.kernel === 'fenger') {
				if (!fengerChunkTmp || fengerChunkTmp.length !== accumulator.length) {
					fengerChunkTmp = allocBuffer(accumulator.length);
				}
				fengerChunkTmp.fill(0);
				bindingRef.compute_recovery_fenger(
					chunkInputs.subarray(0, chunkBlockIdx * blockSize),
					fengerChunkTmp,
					chunkBlockIdx,
					numRecovery,
					blockSize,
					chunkStartGlobalBlock,
					firstRecovery,
					self.opts.numThreads || 0
				);
				for (var i = 0; i < accumulator.length; i++) {
					accumulator[i] ^= fengerChunkTmp[i];
				}
			} else {
				bindingRef.compute_recovery_accumulate(
					chunkInputs,
					accumulator,
					chunkBlockIdx,
					numRecovery,
			...
			chunkStartGlobalBlock += chunkBlockIdx;
			chunkBlockIdx = 0;
		}
		...
		if (!useChunkedCreate && gateAllowsMatvec && !useStreamingCreate && bindingRef && typeof bindingRef.build_coefficient_matrix === 'function'
		...
		try {
			...
			// gated at capture).
			if (useChunkedCreate) {
				// Chunked recovery accumulation completed during read phase in-place in accumulator.
		...
		global.__par3ProfEnd('kernel');
			...
			self._finalizeRecoveryBlocks(finalAcc, totalInputBlocks, eventCb, function(err) {
				...
				self._outputStream.end(function() {
					self._outputStream = null;
					eventCb('complete', { processedBlocks: processedBlocks, usedStreamingRecovery: streamRecoveryUsed, usedChunkedRecovery: useChunkedCreate });
		...
		var processFileFull = function(idx) {
			...
			// Skip bulk-read for files >= 2^32: Buffer.copy internally truncates
			// source/dest offsets to uint32, causing block data to wrap at the 4 GiB
			// boundary (e.g., block 4096 at source offset 4 GiB reads from offset 0).
			// The per-block fs.readSync path uses libuv's 64-bit positions and handles
			// this correctly.
			try {
				fd = fs.openSync(file.name, 'r');
				var fileStat = fs.fstatSync(fd);
				if(useChunkedCreate || fileStat.size >= 0x100000000) throw new Error('bulk-read-skip-large-file');
				bulkFileBuf = Buffer.alloc(fileStat.size);
				fs.readSync(fd, bulkFileBuf, 0, fileStat.size, 0);
				fs.closeSync(fd);
				fd = null;
				var fileBlocks = Math.ceil(bulkFileBuf.length / blockSize);
				var fileBlockOffset = processedBlocks * blockSize;
				var copyLen = Math.min(bulkFileBuf.length, fullInputs.length - fileBlockOffset);
				bulkFileBuf.copy(fullInputs, fileBlockOffset, 0, copyLen);
				var paddedBytes = fileBlocks * blockSize;
				if (bulkFileBuf.length < paddedBytes && fileBlockOffset + paddedBytes <= fullInputs.length) {
					fullInputs.fill(0, fileBlockOffset + bulkFileBuf.length, fileBlockOffset + paddedBytes);
				}
				...
				fd = fs.openSync(file.name, 'r');
				remaining = file.size;
				// The per-block fallback reads asynchronously AFTER this
				// point — a post-stat here would still race the reads.
				// Invalidate the streaming recovery outright on the
				// fallback path (rare: bulk read failed) so recovery can
				// never be finalized against re-read data.
				streamRecovery = null;
		...
		function readBatch() {
					...
					while(items.length < batchSize && remaining > 0) {
						...
						if(bulkFileBuf) {
							...
							if(HASH_POOL_ENABLED) {
								// T9 (issue #59): block bytes land directly in
								// this packet's body window of the reusable
								// slab — no per-block Buffer.alloc.
								buf = readSlab.subarray(
									items.length * pktCap + 56,
									items.length * pktCap + 56 + toRead);
							} else {
								buf = acquireInputBuffer(toRead);
							}
							bulkFileBuf.copy(buf, 0, offset, offset + toRead);
						} else if(HASH_POOL_ENABLED) {
							// T9 (issue #59): body window; the shared
							// fs.readSync below fills it in place. The old
							// per-block Buffer.alloc existed because A3 pool
							// slices alias the pool and would be overwritten
							// mid-batch — slab windows are private per slot,
							// so that hazard is gone.
							buf = readSlab.subarray(
								items.length * pktCap + 56,
								items.length * pktCap + 56 + toRead);
						} else {
						...
						if(useChunkedCreate) {
							var chunkOffset = chunkBlockIdx * blockSize;
							chunkBlockIdx++;
							buf.copy(chunkInputs, chunkOffset);
							if(buf.length < blockSize) {
								chunkInputs.fill(0, chunkOffset + buf.length, chunkOffset + blockSize);
							}
							if(chunkBlockIdx === chunkBlocks) {
								flushChunkAccumulator();
							}
						} else if(!bulkFileBuf) {
				...
				// are read straight into each packet's body window:
				//   bulk path : bulkFileBuf -> slab[56..]
				//   sync path : fs.readSync(fd, slab, 56, ...)
...
function fileInfo(files, recurse, skipSymlinks, concurrency, cb) {
	...
	scanFiles = function(files, recurse, cbDoneScan) {
		...
		// cubic review 41b8069e P1: async.eachLimit is compatible with declared async 0.2.x - 2.x
			...
			bufs.get(function(buf) {
				...
				async.waterfall([
					...
					function(stat, cb) {
						...
						if(stat.isDirectory()) {
							...
							if(recurse) {
								fs.readdir(file, function(err, dirFiles) {
									// release buffer before recursive scan
						...
						if(!stat.isFile()) {
							// cubic review 41b8069e P1: clear info so final callback does not double-put or store non-file
							info = null;
					...
					// cubic review 41b8069e P1: release scan buffer exactly once per file item
					if(bufHeld) {
						bufs.put(buf);
						bufHeld = false;
					}
...
function par3_create(inputFiles, outputBase, opts, cb) {
	...
	var ee = run_par3(inputFiles, blockSize, {
	...
	// cubic review df1de4cb P2: guard ee in case run_par3 returned undefined (e.g. invalid inputFiles)
	if (ee && opts && typeof opts.onEvent === 'function') {
		ee.on('complete', function(par, data) { opts.onEvent('complete', data); });
		ee.on('progress', function(par, proc, tot) { opts.onEvent('progress', proc, tot); });
	}
	return ee;
}
