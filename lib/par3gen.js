...
function _dispatchFromQueueTo(worker) {
	...
	while(hashPool.queue.length > 0 && !worker.busy) {
		...
		worker.cb = item.cb;
		try {
			// Same Buffer.from copy as _hashBlockParallel — item.blockData
			// may be a slab-backed subarray view, so we copy to a fresh
			// ArrayBuffer before transferring. The previous code put a
			// Buffer in the transferList directly, which throws
			// "Found invalid value in transferList" on Node 22+ and left
			// the remaining queue items stranded (the worker never sees a
			// message, never responds, and _dispatchFromQueueTo is never
			// called again for it — the test hangs).
			var transferBuf = Buffer.from(item.blockData);
			worker.postMessage({
...
function _hashBlockParallel(blockIndex, blockData, afterChecksum, cb) {
	...
	if(found) {
		...
		found.cb = cb;
		try {
			// cubic P2: always copy to a fresh Buffer before transferring.
			// slab-backed views alias the per-batch slab ArrayBuffer; passing
			// the slab's underlying buffer in the transferList detaches the
			// slab on the main thread while the worker is still running, so
			// the next readBatch constructs Buffer objects on a detached
			// ArrayBuffer ("Cannot perform Construct on a detached
			// ArrayBuffer"). Buffer.from() makes a fresh ArrayBuffer so the
			// transfer is safe. User-supplied Buffers that aren't aliases
			// are unaffected — Buffer.from copies a plain Buffer too, which
			// is correct but slightly wasteful; perf is dominated by the
			// block hashing anyway.
			var transferBuf = Buffer.from(blockData);
			found.postMessage({
