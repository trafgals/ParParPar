...
function dispatchRecovery(binding, inputs, outputs, numInputs, numRecovery, blockSize, firstInput, firstRecovery, numThreads, fallbackFn) {
	// Fenger dispatch (issue #46 K3): Fenger's per-word cost is
	// O(M(N)·log²N + R) — the Bostan-Schost MPE (interpolation + evaluation)
	// is R-independent, but the final divide-by-V(y_r) is O(R). Barycentric's
	// per-word cost is O(N·R) (direct Cauchy matvec). Fenger wins only when
	// M(N)·log²N < R·(N-1), i.e. R > M(N)·log²N / (N-1). For N=16384 that
	// crossover is R ≈ 5832 (empirically: Fenger loses to Barycentric at
	// every power-of-2 R measured up to 4096). The magic shape (R=8) is
	// ~15× slower on Fenger (21 vs 342 MB/s at 16G/262144). So: prefer
	// Barycentric for R < FENGER_MIN_R; use Fenger only for R >=
	// FENGER_MIN_R (where its R-independence amortizes) or when explicitly
	// forced via PAR3_GF64_USE_FENGER=1.
	//
	// Cost-model crossover (calibrated 2026-08-18 on Zen4, N=16384, 64KiB):
	//   R=8:    Fenger 36.15 vs Bary 324.15 MB/s  (Bary 9.0× faster)
	//   R=64:   Fenger 26.51 vs Bary 227.76 MB/s  (Bary 8.6× faster)
	//   R=256:  Fenger 13.70 vs Bary 106.20 MB/s  (Bary 7.8× faster)
	//   R=512:  Fenger  8.15 vs Bary  68.33 MB/s  (Bary 8.4× faster)
	//   R=1024: Fenger  4.58 vs Bary  38.71 MB/s  (Bary 8.5× faster)
	//   R=2048: Fenger  2.62 vs Bary  62.50 MB/s  (Bary 24× faster)
	//   R=4096: Fenger TMO  vs Bary  10.13 MB/s  (Fenger times out)
	// Fenger NEVER beats Barycentric at any measured power-of-2 R. The
	// theoretical crossover (M(N)·log²N / (N-1), with M(N)≈2N for
	// Bostan-Schost MPE) is R ≈ 5832, so the first power-of-2 R where
	// Fenger could amortize is 8192. In practice this is an extreme
	// workload (R=8192 on N=16384); all practical shapes use Barycentric.
	//
	// FENGER_MIN_R: the minimum R for which Fenger is preferred over
	// Barycentric. Set to 8192 (the first power-of-2 R at or above the
	// theoretical crossover). Override via PAR3_FENGER_MIN_R env.
	//
	// Env contract:
	...
	//   unset                                    -> cost-model dispatch
	//                                               (Barycentric for R <
	//                                                FENGER_MIN_R, Fenger for
	//                                                R >= FENGER_MIN_R)
		//   PAR3_FENGER_MIN_R                        -> override the crossover R
		//                                               (default 8192)
	...
	// Cost-model gate (issue #96 P0): Fenger's per-word cost is
	// O(M(N)·log²N + R) vs Barycentric O(N·R). Fenger wins only when
	// R > M(N)·log²N / (N-1). For N=16384 the theoretical crossover is
	// R ≈ 5832 (M(N)≈2N for Bostan-Schost MPE); empirically Fenger loses
	// to Barycentric at every power-of-2 R measured up to 4096 (see the
	// calibration table above). We set FENGER_MIN_R=8192 — the first
	// power-of-2 R at or above the theoretical crossover — so that
	// Barycentric is the default for all practical workloads and Fenger
	// is only tried at extreme R where its R-independence could amortize.
	var fengerMinR = parseInt(process.env.PAR3_FENGER_MIN_R, 10);
	if(!isFinite(fengerMinR) || fengerMinR <= 0) fengerMinR = 8192;
	var fengerCostFavorable = numRecovery >= fengerMinR;
	...
	if(fengerEligible && !fengerDisabled) {
		if(!_dispatchRecoveryWarned) {
			...
			console.log('[par3gen] using Fenger kernel for numInputs=' + numInputs + ', numRecovery=' + numRecovery + ' (R >= FENGER_MIN_R=' + fengerMinR + ')');
		}
		// T11 (issue #91): chunked-input path. When the inputs buffer
		// would exceed the cap (PAR3_INPUT_CHUNK_CAP for testing / future
		// production, or kMaxLength by default), split into per-chunk
		// calls and XOR the per-chunk recovery outputs. The Fenger matvec
		// is XOR-linear (f(A xor B) = f(A) xor f(B) over GF(2^64)) so the
		// chunked result is bit-equal to the single-call result.
		var PAR3_INPUT_CHUNK_CAP = parseInt(process.env.PAR3_SIMULATED_BUFFER_CAP, 10);
		if(!isFinite(PAR3_INPUT_CHUNK_CAP) || PAR3_INPUT_CHUNK_CAP <= 0) {
			PAR3_INPUT_CHUNK_CAP = parseInt(process.env.PAR3_INPUT_CHUNK_CAP, 10);
		}
		var kMaxLength = (require('buffer').kMaxLength || (1024*1024*1024-1)) - 1024 - 68;
		var inputCap = (isFinite(PAR3_INPUT_CHUNK_CAP) && PAR3_INPUT_CHUNK_CAP > 0)
			? PAR3_INPUT_CHUNK_CAP : kMaxLength;
		if (inputs.length > inputCap) {
			// cubic P2: align chunks to blockSize (not 8) — Fenger binding's
			// numInputs*blockSize invariant. Smaller-than-one-block caps fall
			// through to the single-call path.
			if (blockSize <= 0 || inputCap < blockSize) {
				return binding.compute_recovery_fenger(
					inputs, outputs, numInputs, numRecovery, blockSize,
					firstInput, firstRecovery, numThreads
				);
			}
			var chunkSizeBytes = Math.floor(inputCap / blockSize) * blockSize;
			var numChunks = Math.ceil(inputs.length / chunkSizeBytes);
			outputs.fill(0);
			for (var ci = 0; ci < numChunks; ci++) {
				var start = ci * chunkSizeBytes;
				var end = Math.min(start + chunkSizeBytes, inputs.length);
				var chunkIn = inputs.slice(start, end);
				var chunkOut = Buffer.alloc(outputs.length);
binding.compute_recovery_fenger(
				chunkIn, chunkOut,
				chunkIn.length / blockSize,
				numRecovery, blockSize,
				firstInput + (start / blockSize), firstRecovery,
				numThreads
				);
				for (var oi = 0; oi < outputs.length; oi++) {
					outputs[oi] ^= chunkOut[oi];
				}
			}
			return;
		}
	...
	if(useBarycentric) {
		if(!_dispatchRecoveryWarned) {
			_dispatchRecoveryWarned = true;
			console.log('[par3gen] using Barycentric kernel for numInputs=' + numInputs + ', numRecovery=' + numRecovery + (fengerCostFavorable ? '' : ' (R=' + numRecovery + ' < FENGER_MIN_R=' + fengerMinR + ')'));
		}
...
// ============================================================================
// T1 (issue #59): deterministic kernel-routing gate
// ----------------------------------------------------------------------------
//
// Replaces the dense-matrix-OOM accident with an explicit per-shape
// cost-model selection, called BEFORE the coeff worker spawn. The
// dense-matrix-OOM path stays as a defensive fallback (if the cost model
// said matvec but the worker still OOMs, dispatchRecovery picks the
// next-best kernel). PAR3_FENGER_MIN_R remains the Fenger eligibility
// floor; PAR3_GF64_USE_FENGER=1 forces Fenger; PAR3_GF64_USE_FENGER=0
// disables it (existing contract).
//
// Cost model (per create; lower = cheaper):
//   matvec   = R * N * blockSize + R * N * 8       (byte-traffic + dense R×N matrix footprint)
//   fenger   = (N + R) * log2(N + R)^2 * FENGER_FIELD_OP_COST + R * blockSize
//                                                (Bostan-Schost MPE field ops + one streaming pass)
//   bary     = N * R * blockSize                   (direct Cauchy matvec; same as matvec's
//                                                  byte-traffic, no matrix-build cost)
//
// Tie-break order (lower index wins ties): fenger < barycentric < matvec
//
// Selection (deterministic):
//   1. fenger eligible AND cost(fenger) <= cost(matvec) AND cost(fenger) <= cost(bary) -> fenger
//   2. matvec feasible  AND cost(matvec) <= cost(bary) -> matvec
//   3. barycentric eligible -> barycentric
//   4. else -> fallback (compute_recovery_full or per-batch)
//
// Eligibility:
//   - fenger:    R is 0/1/power-of-2 AND binding has compute_recovery_fenger
//                AND (R >= PAR3_FENGER_MIN_R OR matvec infeasible)
//   - matvec:    R * N * 8 <= MATVEC_MAX_BYTES (=128 MiB) AND
//                binding has compute_recovery_with_coeff AND build_coefficient_matrix
//   - bary:      N > BARY_MIN_INPUTS AND R > 0 AND blockSize % 8 == 0 AND
//                binding has compute_recovery_barycentric
//
// Fenger override: when matvec is infeasible (matrix footprint exceeds
// MATVEC_MAX_BYTES), the PAR3_FENGER_MIN_R gate is LOWERED for the
// affected shape. Rationale: dense-matrix allocation would have OOM'd at
// runtime; the cost model preempts the OOM by routing to Fenger (or, if
// R isn't power-of-2 for Fenger, to Barycentric).
//
// At pow2 bench geometries (N>=65536, R=N/8>=8192): fenger is ALWAYS
// selected without waiting for an allocation failure. This is the
// regression the gate fixes — before T1, Fenger engagement was an OOM
// accident on these shapes.
//
// PAR3_ROUTING_DEBUG=1 prints
// "[par3gen] routing shape N=..,R=..,B=.. -> <kernel> (reason)" on every
// decideRecoveryKernel() call.
// ============================================================================

var MATVEC_MAX_BYTES = 128 * 1024 * 1024;
var FENGER_FIELD_OP_COST = 5000;

function _costMatvec(N, R, blockSize) {
	var matrixBytes = R * N * 8;
	var byteTraffic = R * N * blockSize;
	return {
		matrixBytes: matrixBytes,
		byteTraffic: byteTraffic,
		total: matrixBytes + byteTraffic,
		feasible: matrixBytes <= MATVEC_MAX_BYTES
	};
}

function _costFenger(N, R, blockSize) {
	if(!_isPowerOfTwoOrTrivial(R)) {
		return { total: Infinity, eligible: false, reason: 'R=' + R + ' not power-of-2' };
	}
	var nr = N + R;
	var log2nr = nr <= 1 ? 0 : Math.log2(nr);
	var fieldOps = nr * log2nr * log2nr;
	var streamingPass = R * blockSize;
	return {
		fieldOps: fieldOps,
		streamingPass: streamingPass,
		total: fieldOps * FENGER_FIELD_OP_COST + streamingPass,
		eligible: true
	};
}

function _costBary(N, R, blockSize) {
	var byteTraffic = N * R * blockSize;
	// cubic P2: honor PAR3_BF64_MIN_INPUTS env override (default 10000);
	// matches the legacy _dispatchRecovery path so the T1 routing gate
	// (decideRecoveryKernel) doesn't silently ignore user-configured thresholds.
	var baryMinInputs = parseInt(process.env.PAR3_BF64_MIN_INPUTS, 10);
	if(!isFinite(baryMinInputs) || baryMinInputs <= 0) baryMinInputs = BARY_MIN_INPUTS_DEFAULT;
	var eligible = N > baryMinInputs && R > 0 && blockSize % 8 === 0;
	return {
		byteTraffic: byteTraffic,
		total: byteTraffic,
		eligible: eligible,
		reason: eligible ? null : 'N=' + N + ' <= BARY_MIN or R=' + R + '=0 or B%8!=0'
	};
}

// Pick the recovery kernel for a given (N, R, blockSize). Returns
// { kernel, reason, costs, matrixBytes, ...eligibility flags }.
// Honors PAR3_FENGER_MIN_R, PAR3_GF64_USE_FENGER, PAR3_ROUTING_DEBUG.
// Deterministic given the same inputs + env (no randomness, no async).
function decideRecoveryKernel(N, R, blockSize, binding) {
	var fengerMinR = parseInt(process.env.PAR3_FENGER_MIN_R, 10);
	if(!isFinite(fengerMinR) || fengerMinR <= 0) fengerMinR = 8192;
	...
	var matvec = _costMatvec(N, R, blockSize);
	var hasMatvec = binding && typeof binding.compute_recovery_with_coeff === 'function'
		&& typeof binding.build_coefficient_matrix === 'function';
	var matvecFeasible = matvec.feasible && hasMatvec;
	var matvecReason = !matvec.feasible
		? 'matrix=' + matvec.matrixBytes + 'B > MATVEC_MAX_BYTES=' + MATVEC_MAX_BYTES
		: (!hasMatvec ? 'binding lacks compute_recovery_with_coeff/build_coefficient_matrix' : 'cost model');

	var fenger = _costFenger(N, R, blockSize);
	var hasFenger = binding && typeof binding.compute_recovery_fenger === 'function';
	var fengerCostFavorable = R >= fengerMinR || !matvec.feasible;
	var fengerEligible = fenger.eligible && hasFenger && (fengerForced || fengerCostFavorable);
	var fengerReason;
	if(!fenger.eligible) fengerReason = fenger.reason;
	else if(!hasFenger) fengerReason = 'binding lacks compute_recovery_fenger';
	else if(!fengerForced && !fengerCostFavorable) fengerReason = 'R=' + R + ' < FENGER_MIN_R=' + fengerMinR + ' and matvec feasible';
	else fengerReason = fengerForced ? 'forced (PAR3_GF64_USE_FENGER=1)' : 'cost model';

	var bary = _costBary(N, R, blockSize);
	var hasBary = binding && typeof binding.compute_recovery_barycentric === 'function';
	var baryEligible = bary.eligible && hasBary;
	var baryReason = !bary.eligible
		? bary.reason
		: (!hasBary ? 'binding lacks compute_recovery_barycentric' : 'cost model');

	var chosen;
	if(fengerDisabled) {
		if(matvecFeasible) chosen = { kernel: 'matvec', reason: 'fenger disabled (PAR3_GF64_USE_FENGER=0)' };
		else if(baryEligible) chosen = { kernel: 'barycentric', reason: 'fenger disabled (PAR3_GF64_USE_FENGER=0)' };
		else chosen = { kernel: 'fallback', reason: 'no eligible kernel (fenger disabled)' };
	} else if(fengerForced && fengerEligible) {
		chosen = { kernel: 'fenger', reason: fengerReason };
	} else {
		var prio = { fenger: 0, barycentric: 1, matvec: 2 };
		var candidates = [];
		if(fengerEligible) candidates.push({ kernel: 'fenger', cost: fenger.total, reason: fengerReason });
		if(matvecFeasible) candidates.push({ kernel: 'matvec', cost: matvec.total, reason: matvecReason });
		if(baryEligible) candidates.push({ kernel: 'barycentric', cost: bary.total, reason: baryReason });
		if(candidates.length === 0) {
			chosen = { kernel: 'fallback', reason: 'no eligible kernel' };
		} else {
			candidates.sort(function(a, b) {
				if(a.cost !== b.cost) return a.cost - b.cost;
				return prio[a.kernel] - prio[b.kernel];
			});
			chosen = candidates[0];
		}
	}
	...
	var result = {
		...
		costs: { matvec: matvec.total, fenger: fenger.total, barycentric: bary.total },
		matrixBytes: matvec.matrixBytes,
		matvecFeasible: matvecFeasible,
		fengerEligible: fengerEligible,
		baryEligible: baryEligible
	};

	var dbg = process.env.PAR3_ROUTING_DEBUG;
	if(dbg === '1' || dbg === 'true' || dbg === 'yes' || dbg === 'on') {
		console.log('[par3gen] routing shape N=' + N + ',R=' + R + ',B=' + blockSize +
			' -> ' + result.kernel + ' (reason: ' + result.reason + ')' +
			' costs: matvec=' + result.costs.matvec + ' fenger=' + result.costs.fenger +
			' bary=' + result.costs.barycentric +
			' matrixBytes=' + result.matrixBytes);
	}

	return result;
}

// Issue #115: Adaptive chunk sizing for streaming recovery accumulation.
// Issue #117: Optimal default chunk capacity is 64 MiB across all archive shapes.
// Issue #119: Topology-aware multi-CPU scaling & L3 cache-adaptive chunk sizing.
// Fits working set within 2/3 of L3 cache per cluster/CCD while preserving the 64 MiB sweet spot
// on Ryzen 7800X3D (96 MiB L3) and bounding RSS.
function getCpuTopology() {
	var binding = getGf64Binding();
	if (binding && typeof binding.get_cpu_topology === 'function') {
		try {
			var topo = binding.get_cpu_topology();
			if (topo && typeof topo === 'object' && topo.physicalCores > 0) {
				return topo;
			}
		} catch (e) {}
	}
	...
	return {
		physicalCores: numCpus,
		logicalCores: numCpus,
...
function resetCpuTopologyCache() {
	var binding = getGf64Binding();
	if (binding && typeof binding.reset_cpu_topology_cache === 'function') {
		try {
			binding.reset_cpu_topology_cache();
		} catch (e) {}
	}
}

function getOptimalChunkCap(topology, blockSize) {
	var topo = topology || getCpuTopology();
	var ccdL3Bytes = (topo && topo.l3PerCluster > 0) ? topo.l3PerCluster : (32 * 1024 * 1024);

	// Rule: chunk should comfortably fit within 2/3 of the L3 cache sharing domain
	// to leave room for the recovery accumulator and OS buffers without evicting cache lines.
	var optimalChunk = Math.floor(ccdL3Bytes * (2 / 3));

	// Clamp to [16 MiB, 64 MiB], aligned to block size
	var clampedChunk = Math.min(64 * 1024 * 1024, Math.max(16 * 1024 * 1024, optimalChunk));

	// Align to blockSize
	var bs = blockSize || (64 * 1024);
	if (bs > 0 && clampedChunk > bs) {
		clampedChunk = Math.floor(clampedChunk / bs) * bs;
	} else if (bs > clampedChunk) {
		clampedChunk = bs;
	}
	return clampedChunk;
}

function decideChunkCapBytes(totalInputBytes, numRecovery, blockSize, kernel, memoryLimit, topology) {
	var chunkCapBytes = getOptimalChunkCap(topology, blockSize);

	var hasExplicitChunkCap = false;
	if (process.env.PAR3_STREAM_CHUNK_BYTES) {
		var parsedChunk = parseInt(process.env.PAR3_STREAM_CHUNK_BYTES, 10);
		if (isFinite(parsedChunk) && parsedChunk > 0) { chunkCapBytes = parsedChunk; hasExplicitChunkCap = true; }
	} else if (process.env.PAR3_SIMULATED_BUFFER_CAP) {
		var parsedSim = parseInt(process.env.PAR3_SIMULATED_BUFFER_CAP, 10);
		if (isFinite(parsedSim) && parsedSim > 0) { chunkCapBytes = parsedSim; hasExplicitChunkCap = true; }
	} else if (process.env.PAR3_INPUT_CHUNK_CAP) {
		var parsedCap = parseInt(process.env.PAR3_INPUT_CHUNK_CAP, 10);
		if (isFinite(parsedCap) && parsedCap > 0) { chunkCapBytes = parsedCap; hasExplicitChunkCap = true; }
	}

	var isFenger = (kernel === 'fenger');
	var recoveryBufferReserve = (isFenger ? 2 : 1) * (numRecovery || 0) * (blockSize || 0);
	var defaultBufferBudget = 384 * 1024 * 1024;
	try {
		var totalMem = os.totalmem ? os.totalmem() : 0;
		if (totalMem > 0 && totalMem < 8 * 1024 * 1024 * 1024) {
			// On systems with < 8 GiB RAM, scale budget down proportionally
			defaultBufferBudget = Math.min(defaultBufferBudget, Math.max(64 * 1024 * 1024, Math.floor(totalMem / 16)));
		}
	} catch (e) {}

	var envMemLimit = parseInt(process.env.PAR3_MEMORY_LIMIT, 10);
	var maxMemoryBudget = (isFinite(envMemLimit) && envMemLimit > 0)
		? envMemLimit
		: (memoryLimit || 0);

	var minCap = blockSize || 1;
	var targetBudget = (maxMemoryBudget > 0) ? maxMemoryBudget : defaultBufferBudget;
	var availBudget = targetBudget - recoveryBufferReserve;
	if (availBudget < minCap) {
		// Cubic review P1: Unconditional reserve check -- even with explicit chunk cap,
		// if the memory budget cannot fit the recovery reserve plus one input block,
		// chunked recovery cannot fit without exceeding the limit.
		return 0;
	}

	if (maxMemoryBudget > 0) {
		chunkCapBytes = Math.min(chunkCapBytes, Math.max(minCap, Math.floor(maxMemoryBudget / 2)), availBudget);
	} else {
		chunkCapBytes = Math.min(chunkCapBytes, availBudget);
	}
	return chunkCapBytes;
}
...
// create call. acquireInputBuffer() throws a clear error if the requested
// size exceeds the pool.
...
function acquireInputBuffer(size) {
	...
	if(inputBufferPoolOffset + size > INPUT_BUFFER_POOL_SIZE) {
		// Wrap around: prior slices have already been consumed synchronously
...
// T1: auto-size the input buffer pool to the workload, capped at 64 MiB.
// Slices are consumed synchronously by the caller, so a 64 MiB ring buffer
// avoids allocating multi-gigabyte ghost buffers that cause massive swapping.
function _ensureInputPool(totalSize) {
	if(inputBufferPool !== null) return;
	var poolSize = Math.min(64 * 1024 * 1024, Math.max(1024 * 1024, totalSize + 64 * 1024));
...
function _dispatchFromQueueTo(worker) {
	...
	while(hashPool.queue.length > 0 && !worker.busy) {
		...
		worker.cb = item.cb;
		try {
			// Same Buffer.allocUnsafeSlow + .copy() pattern as
			// _hashBlockParallel — item.blockData may be a slab-backed
			// subarray view AND must end up in a non-pooled ArrayBuffer
			// so the transfer is safe (cubic P1 review):
			//   * Buffer.from(view) at small sizes (≤ poolSize=8KB) is
			//     satisfied from Node's internal buffer pool, whose
			//     .buffer is the 8KB pool slab — transferring it would
			//     detach the pool and break every other pooled Buffer
			//     in the process. postMessage rejects the transfer with
			//     "Cannot transfer object of unsupported type."
			//   * Buffer.allocUnsafeSlow(N) always allocates a fresh
			//     ArrayBuffer of exactly N bytes (no pool fallback), so
			//     its .buffer is safely transferable.
			// The previous Buffer.from() copy + [transferBuf.buffer]
			// worked at the test geometry (64KB-1MB blocks) but regressed
			// at small block sizes (1024-2048 bytes) per the cubic P1.
			var transferBuf = Buffer.allocUnsafeSlow(item.blockData.length);
			item.blockData.copy(transferBuf);
			worker.postMessage({
				blockIndex: item.blockIndex,
				blockData: transferBuf,
...
function _hashBlockParallel(blockIndex, blockData, afterChecksum, cb) {
	...
	if(found) {
		...
		found.cb = cb;
		try {
			// cubic P1 + P2: copy blockData to a fresh, non-pooled
			// ArrayBuffer before transferring.
			//   * (P1) Buffer.from(view) at small sizes (≤ poolSize=8KB)
			//     hands back a pooled Buffer whose .buffer is the 8KB
			//     pool slab — transferring it would detach the pool
			//     and break every other pooled Buffer in the process.
			//     postMessage rejects with "Cannot transfer object of
			//     unsupported type" and the create hangs.
			//   * (P2) Buffer.allocUnsafeSlow(N) always allocates a
			//     fresh ArrayBuffer of exactly N bytes (no pool
			//     fallback), so its .buffer is safely transferable.
			//   * (cubic P2) The slab-detach fix (was: isBuffer ?
			//     blockData : Buffer.from(blockData)) is now subsumed:
			//     Buffer.allocUnsafeSlow always creates a new ArrayBuffer,
			//     so even when blockData aliases the per-batch slab
			//     ArrayBuffer, transferring transferBuf.buffer detaches
			//     only the copy — the slab stays attached on the main
			//     thread and the next readBatch can construct Buffers
			//     on it without "Cannot perform Construct on a detached
			//     ArrayBuffer".
			var transferBuf = Buffer.allocUnsafeSlow(blockData.length);
			blockData.copy(transferBuf);
			found.postMessage({
...
// Fill a PAR3 packet header into an existing buffer (T9 issue #59).
// Layout (48 bytes): magic[0..8] | checksum[8..24] (zeroed placeholder) |
// length LE64[24..32] | inputSetId[32..40] | type ASCII[40..48].
// Single source of truth for the header layout: createPacketHeader and the
// readBatch slab path both delegate here, so the two can never diverge.
function fillPacketHeader(header, type, bodySize, inputSetId) {
	// Magic
...
function createPacketHeader(type, bodySize, inputSetId) {
	...
	fillPacketHeader(header, type, bodySize, inputSetId);
	return header;
}
...
function PAR3Gen(fileInfo, blockSize, opts) {
	...
	// Calculate number of blocks: each file consumes ceil(file.size / blockSize) blocks
	// (cubic review 0c8cc30f P1: account for per-file block alignment)
	var totalBlocks = 0;
	fileInfo.forEach(function(file) {
		totalBlocks += Math.ceil(file.size / o.blockSize);
	});
...
if(o.minRecoverySlices !== null) {
		...
		if(typeof o.minRecoverySlices === 'number' && minRec > 1) minRec = pow2up_clamp(minRec);
	...
	if(o.maxRecoverySlices !== null) {
		...
		if(typeof o.maxRecoverySlices === 'number' && maxRec > 1) maxRec = pow2up_clamp(maxRec);
		o.recoverySlices = Math.min(o.recoverySlices, maxRec);
	}


	// Pow2 contract (issue #59 C2): the Fenger/Bostan-Schost fast path
	// requires a power-of-2 recovery count. Validate the explicit count is
	// a finite non-negative integer (cubic review on #87 P2.1), then check
	// its power-of-two property arithmetically (a NaN check first catches
	// fractional/negative/string coercions before the bit-test). A non-pow2
	// value triggers a clear error directing users to PAR2 for arbitrary
	// counts.
	if(typeof o.recoverySlices !== 'number' || !isFinite(o.recoverySlices) ||
	   o.recoverySlices < 0 || Math.floor(o.recoverySlices) !== o.recoverySlices)
		throw new Error('Recovery slice count must be a finite non-negative integer (got ' + o.recoverySlices + ')');
	if(o.recoverySlices > 1 && (o.recoverySlices & (o.recoverySlices - 1)) !== 0)
		throw new Error('Recovery slice count must be a power of 2 (got ' + o.recoverySlices + '); use PAR2 for arbitrary slice counts');
...
PAR3Gen.prototype = {
	...
	_calcRecoverySlices: function(spec) {
		...
		// Pow2 contract (issue #59 C2): the Fenger/Bostan-Schost fast path
		// requires a power-of-2 recovery count. Proportional specs (ratio/
		// bytes) ceil to the next power of 2 (never less coverage than
		// requested; cubic review on #87 P2.5). Explicit COUNT specs
		// (numeric, 'slices'/'count') keep the raw value — the
		// constructor's pow2 check rejects non-pow2 counts (arbitrary
		// counts are a PAR2 feature).
		var pow2up = function(r) {
			// NaN/Infinity guard: bit-tests on NaN coerce to 0 (false pass).
			if(!isFinite(r) || r <= 1) return r;
			if((r & (r - 1)) === 0) return r;
			// Use arithmetic rather than `1 << (32 - clz32(r-1))`:
			// the shift wraps to negative for r > 2^31 (cubic review
			// on #87 P2.4). Doubling is bounded by r itself.
			var p = 1;
			while(p < r) p <<= 1;
			return p;
		};
		...
		if(spec.unit === 'ratio') {
		return pow2up(Math.ceil(scale * this.totalBlocks * spec.value));
		}
		if(spec.unit === 'slices' || spec.unit === 'count') {
		return Math.round(scale * spec.value);
		}
		if(spec.unit === 'bytes') {
		return pow2up(Math.ceil(scale * (spec.value / this.opts.blockSize)));
		}
		return Math.round(scale * spec.value);
	},
	...
	_createStartPacket: function() {
		...
		// body[1..15] is "reserved" (documented at top of this function as
		// zero-filled) but allocated via allocUnsafe above, so carries junk.
		header.fill(0, 48 + 1, 48 + 16);                                    // body[1..15] = 0
	...
	_createRecoveryPackets: function(recoveryData, cb) {
		var self = this;
		var numRecovery = recoveryData.length;

		// T10 (issue #59): native packet assembly binding. Pack all recovery
		// blocks into one contiguous buffer and hand them to the C++ side,
		// which assembles + BLAKE3-checksums every packet in one shot.
		// Falls back to the per-block JS path below when the binding isn't
		// present (stub builds, e.g. macOS arm64 / Windows where the
		// Gf64Encoder_create typeof guard already kicks in elsewhere).
		var binding = (typeof getGf64Binding === 'function') ? getGf64Binding() : null;
		if(numRecovery > 0 && binding && typeof binding.assemble_recovery_packets === 'function') {
			var blockSize = recoveryData[0].data.length;
			var firstBlockIndex = recoveryData[0].blockIndex;
			var packed = Buffer.alloc(numRecovery * blockSize);
			for(var i = 0; i < numRecovery; i++) {
				recoveryData[i].data.copy(packed, i * blockSize);
			}
			var combined = binding.assemble_recovery_packets(packed, self.inputSetId, firstBlockIndex, numRecovery, blockSize);
			var pktSize = 64 + blockSize;
			var packets = new Array(numRecovery);
			for(var i = 0; i < numRecovery; i++) {
				packets[i] = {
					type: 'recovery',
					data: Buffer.from(combined.subarray(i * pktSize, (i + 1) * pktSize))
				};
			}
			return process.nextTick(function() { cb(null, packets); });
		}
	...
	_processRecoveryBatch: function(batch, firstInputIdx, firstRecovery, numRecovery, accumulator) {
		...
		for(var b = 0; b < bufferCount; b++) {
			...
			// zero-pad invariant (same class as the single-call fullInputs
			// P0 fix): the kernel consumes blockSize per block — a short
			// last block must not carry an allocUnsafe garbage tail.
			tmpBuffers[b].fill(0);
			}
...
run: function(eventCb, completeCb) {
	...
	global.__par3ProfStart('setup');
		...
		var maxMemoryBudget = (isFinite(envMemLimit) && envMemLimit > 0)
			? envMemLimit
			: (self.opts.memoryLimit || 0);
		var defaultBufferBudget = 384 * 1024 * 1024;
		var targetMemoryBudget = (maxMemoryBudget > 0) ? maxMemoryBudget : defaultBufferBudget;

		var hasExplicitChunkCap = !!(process.env.PAR3_STREAM_CHUNK_BYTES ||
		                             process.env.PAR3_SIMULATED_BUFFER_CAP ||
		                             process.env.PAR3_INPUT_CHUNK_CAP);
		...
		// the accumulator cap / recovery reserve consumes the budget (single C++ call writes numRecovery *
		// blockSize bytes; existing temp-file spill stays on the per-batch
		// path for >=4 GiB cases or when reserve exceeds budget).
		...
		if(computeRecoveryFullBinding && typeof computeRecoveryFullBinding.compute_recovery_full === 'function') {
			var earlyRouting = decideRecoveryKernel(totalInputBlocks, numRecovery, blockSize, computeRecoveryFullBinding);
			var isFenger = (earlyRouting.kernel === 'fenger');
			var recoveryReserve = (isFenger ? 2 : 1) * totalRecoverySize;
			// Cubic review P1: Keep reserve check unconditional -- explicit chunk cap
			// controls chunk size, but cannot permit an allocation that exceeds the memory budget.
			if (recoveryReserve + blockSize <= targetMemoryBudget) {
		...
		// A2-rev: recovery buffer captured from the opt-in streaming call
		// (single-file creates within the in-memory cap). When set, the
		// legacy compute is skipped and this feeds _finalizeRecoveryBlocks.
		var streamRecovery = null;
		var streamRecoveryUsed = false;
		// Async-call ordering: the invalidation sites (fallback path, TOCTOU
		// mismatch) can run BEFORE the streaming callback captures — the
		// capture must never override an invalidation.
		var streamRecoveryInvalidated = false;
		// B1 step 3: the streaming call is ASYNC (napi_async_work) so the
		// JS read/hash pipeline overlaps the kernel. finalizeRecovery waits
		// on this before choosing between the streaming buffer and the
		// legacy compute.
		var streamingCallPending = false;
		var streamingWaiters = null;
		function whenStreamingSettled(cb) {
		if(!streamingCallPending) {
			cb();
			return;
			}
		if(!streamingWaiters) streamingWaiters = [];
		streamingWaiters.push(cb);
		}
		function settleStreamingCall() {
		streamingCallPending = false;
		if(streamingWaiters) {
			var w = streamingWaiters;
			streamingWaiters = null;
		...
		if(useComputeRecoveryFull) {
		// Single-call path: full input buffer (or bounded chunked buffer for
		// large workloads > memoryLimit) + compute_recovery_full / barycentric,
		// then reuse _finalizeRecoveryBlocks / packet streaming.
		var totalInputBytes = totalInputBlocks * blockSize;

		var bindingRef = computeRecoveryFullBinding;

		// T1 (issue #59): deterministic kernel-routing gate. Picks the
		// kernel BEFORE the coeff worker spawn so the OOM-fallback path
		// is no longer the only way Fenger engages for large pow2 shapes.
		// Honors PAR3_FENGER_MIN_R, PAR3_GF64_USE_FENGER, PAR3_ROUTING_DEBUG.
		var routingDecision = decideRecoveryKernel(totalInputBlocks, numRecovery, blockSize, bindingRef);
		var gateAllowsMatvec = (routingDecision.kernel === 'matvec');

		// Streaming chunked accumulation: when archive size exceeds bounded chunk size
		// or PAR3_FORCE_CHUNKED=1, stream inputs in bounded chunks
		// using compute_recovery_accumulate/barycentric/fenger, eliminating the fullInputs buffer
		// and keeping peak RSS safely <= 450 MiB across all archive shapes (up to 64+ GiB).
		// Adaptive chunk sizing & recovery buffer deduction (Issue #115 / Cubic review P2/P3):
		var chunkCapBytes = decideChunkCapBytes(
			totalInputBytes,
			numRecovery,
			blockSize,
			routingDecision.kernel,
			self.opts.memoryLimit
		);
		if (chunkCapBytes <= 0) {
			throw new RangeError(
				'Cannot schedule chunked recovery: recovery reserve exceeds memory budget (' +
				targetMemoryBudget + ' bytes); cannot allocate input chunk'
			);
		}

		var forceChunked = process.env.PAR3_FORCE_CHUNKED === '1';
		var disableChunked = process.env.PAR3_DISABLE_CHUNKED === '1' || process.env.PAR3_GF64_LEGACY_FULL === '1';
		var isChunkedRecovery = !disableChunked && (forceChunked || totalInputBytes > chunkCapBytes);

		var fullInputs = null;
		var chunkInputs = null;
		var chunkCapacityBlocks = 0;
		var chunkBlockCount = 0;
		var chunkFirstBlockIdx = 0;
		var chunkFlushCount = 0;
		var hasAccumulated = false;

		var nextBlockIdx = 0;

		var fengerChunkScratch = null;
		if(isChunkedRecovery) {
			chunkCapacityBlocks = Math.max(1, Math.floor(chunkCapBytes / blockSize));
			chunkInputs = allocBuffer(chunkCapacityBlocks * blockSize);
		} else {
			fullInputs = allocBuffer(totalInputBlocks * blockSize);
			// Short-file zero-pad invariant: allocBuffer is allocUnsafe —
			// the tail beyond the copied file bytes must be ZERO (the
			// recovery kernel consumes numInputs x blockSize and the
			// streaming path zero-pads; a garbage tail made legacy short-
			// file creates nondeterministic). Zero ONLY the padding region
			// — the copy overwrites the file bytes (cubic review on #82
			// P2: a full-buffer memset is a wasted multi-GiB pass).
			if(self.totalSize < fullInputs.length) fullInputs.fill(0, self.totalSize);
		}

		function flushChunk() {
			if(chunkBlockCount === 0) return;
			chunkFlushCount++;
			eventCb('chunk_flush', {
				chunkIndex: chunkFlushCount,
				chunkBlocks: chunkBlockCount,
				chunkFirstBlockIdx: chunkFirstBlockIdx
			});
			ensureAccumulator();
			var chunkSub = (chunkBlockCount === chunkCapacityBlocks)
				? chunkInputs
				: chunkInputs.subarray(0, chunkBlockCount * blockSize);
			if (routingDecision.kernel === 'barycentric' && typeof bindingRef.compute_recovery_barycentric === 'function') {
				bindingRef.compute_recovery_barycentric(
					chunkSub,
					accumulator,
					chunkBlockCount,
					numRecovery,
			...
			} else if (routingDecision.kernel === 'fenger' && typeof bindingRef.compute_recovery_fenger === 'function') {
				// Cubic review P1: Route Fenger chunks through temporary recovery buffer and XOR into accumulator
				if (!fengerChunkScratch) fengerChunkScratch = allocBuffer(numRecovery * blockSize);
				bindingRef.compute_recovery_fenger(
					chunkSub,
					fengerChunkScratch,
					chunkBlockCount,
					numRecovery,
				...
				// Cubic review P1 (round 4): xor_buffers is the canonical reduction.
				//   It now throws on length mismatch (the previous `min(dstLen, srcLen)`
				//   silently truncated and corrupted the chunked-Fenger recovery packets).
				//   Both buffers are sized numRecovery * blockSize on this path, so
				//   the equal-length contract is guaranteed by construction — assert it.
				if (accumulator.length !== fengerChunkScratch.length) {
					throw new Error(
						'chunked Fenger recovery: accumulator.length (' + accumulator.length +
						') must equal fengerChunkScratch.length (' + fengerChunkScratch.length +
						') — both are numRecovery * blockSize'
					);
				}
				bindingRef.xor_buffers(accumulator, fengerChunkScratch);
			} else {
				// Issue #117: accumulator is guaranteed pre-zeroed by ensureAccumulator().
				// Passing accumulate=true ensures all chunk flushes (including chunk 0) route to
				// output-domain decomposition (Case 3), eliminating thread-local scratch allocations
				// (up to 128 MiB default) and the post-worker XOR reduction per chunk flush.
			...
			hasAccumulated = true;
			chunkFirstBlockIdx += chunkBlockCount;
			chunkBlockCount = 0;
		}
		...
		if (!isChunkedRecovery && gateAllowsMatvec && !useStreamingCreate && bindingRef && typeof bindingRef.build_coefficient_matrix === 'function'
		...
		global.__par3ProfStart('kernel');
		// B1 step 3: the streaming call is async — the kernel phase must
		// wait for it to settle before choosing between the streaming
		// buffer and the legacy compute (whenStreamingSettled runs the
		// callback inline when the call already finished).
		whenStreamingSettled(function() {
		try {
			// A2-rev: the opt-in streaming call already computed the
			// recovery natively — feed it straight into the accumulator
			// and skip the legacy compute (single-file creates only,
			// gated at capture).
			if (streamRecovery) {
				ensureAccumulator();
				streamRecovery.copy(accumulator, 0, 0, Math.min(streamRecovery.length, accumulator.length));
				streamRecovery = null;
				streamRecoveryUsed = true;
			} else if (isChunkedRecovery) {
				flushChunk();
			} else if (routingDecision.kernel === 'fenger' && typeof bindingRef.compute_recovery_fenger === 'function') {
				// T1 (issue #59): gate bypasses dispatchRecovery's FENGER_MIN_R
				// so the cost-model decision (incl. matvec-infeasible override)
				// is authoritative; same binding call = byte-identical output.
				bindingRef.compute_recovery_fenger(
					fullInputs,
					accumulator,
					totalInputBlocks,
			...
			} else if (routingDecision.kernel === 'barycentric' && typeof bindingRef.compute_recovery_barycentric === 'function') {
				// T1 (issue #59): gate picked Barycentric directly.
				bindingRef.compute_recovery_barycentric(
					fullInputs,
					accumulator,
					totalInputBlocks,
			...
			} else if (precomputedCoeff && typeof bindingRef.compute_recovery_with_coeff === 'function') {
		...
		global.__par3ProfEnd('kernel');
			...
			self._finalizeRecoveryBlocks(finalAcc, totalInputBlocks, eventCb, function(err) {
				...
				self._outputStream.end(function() {
					self._outputStream = null;
					eventCb('complete', { processedBlocks: processedBlocks, usedStreamingRecovery: streamRecoveryUsed });
		...
		var processFileFull = function(idx) {
			...
			// TOCTOU guard (cubic review on #78 P1): stat before the
			// streaming read so a source that changes between the two reads
			// (streaming + JS pipeline) can be detected and the streaming
			// recovery invalidated — recovery must match the data packets.
			var streamStat = null;
			...
			if(useStreamingCreate && file.name) {
				// mmap choice passes as an option (the process-global env
				// gate races across concurrent async calls — cubic review
				// on #82 P2); files <= 2 GiB use the mmap read path.
				var streamingUseMmap = file.size <= (2 * 1024 * 1024 * 1024);
				try {
					streamStat = fs.statSync(file.name);
					streamingCallPending = true;
					streamRecoveryInvalidated = false;
					bindingRef.par3_create_streaming(file.name, {
						...
						useMmap: streamingUseMmap
					}, function(streamErr, streamResult) {
						// A2-rev: capture the recovery buffer when the
						// streaming call can be authoritative — single-file
						// create, full-size buffer, within the in-memory cap,
						// not invalidated, and the source still stable
						// (re-stat at capture time: the native call may have
						// opened the file after the JS read — cubic review
						// on #82 P1).
						var captureOk = !streamErr && streamResult && streamResult.recoveryBuffer
						   && !streamRecoveryInvalidated
						   && self.files.length === 1
						   && totalRecoverySize <= MAX_IN_MEMORY
						   && streamResult.recoveryBuffer.length === totalRecoverySize;
						if(captureOk && streamStat) {
							var capStat = null;
							try { capStat = fs.statSync(file.name); } catch (e) { /* keep null */ }
							if(!capStat ||
							   capStat.size !== streamStat.size ||
							   capStat.mtimeMs !== streamStat.mtimeMs) {
								captureOk = false;
							}
						}
						if(captureOk) {
							streamRecovery = streamResult.recoveryBuffer;
						}
						settleStreamingCall();
					});
				} catch(streamEx) {
					if(streamingUseMmap) delete process.env.PAR3_GF64_USE_MMAP;
					// Non-fatal: the legacy compute remains authoritative.
					settleStreamingCall();
				}
			}
			...
			// T9 (issue #59): per-file DATA-packet slab, reused across all
			// read batches of this file (batches are strictly serialized).
			var readSlab = null;
			var bulkReadOk = false;

			// Direct single bulk read into fullInputs for small files (<= 64 MiB).
			// Avoids duplicate Buffer.alloc while maintaining zero-copy into fullInputs
			// and allowing fallback invalidation if bulk read fails.
			try {
				fd = fs.openSync(file.name, 'r');
				var fileStat = fs.fstatSync(fd);
				if(isChunkedRecovery || fileStat.size >= 0x100000000) throw new Error('bulk-read-skip-large-file');
				var fileBlockOffset = nextBlockIdx * blockSize;
				var fileBlocks = Math.ceil(fileStat.size / blockSize);
				var paddedBytes = fileBlocks * blockSize;
				var toRead = Math.min(fileStat.size, fullInputs.length - fileBlockOffset);
				var bulkRead = 0;
				while(bulkRead < toRead) {
					var n = fs.readSync(fd, fullInputs, fileBlockOffset + bulkRead, toRead - bulkRead, bulkRead);
					if(n === 0) break;
					bulkRead += n;
				}
				fs.closeSync(fd);
				fd = null;
				if (bulkRead < paddedBytes && fileBlockOffset + paddedBytes <= fullInputs.length) {
					fullInputs.fill(0, fileBlockOffset + bulkRead, fileBlockOffset + paddedBytes);
				}
				remaining = fileStat.size;
				bulkReadOk = true;
			} catch(e) {
				if(fd) { try{fs.closeSync(fd);}catch(e2){} }
				fd = fs.openSync(file.name, 'r');
				remaining = file.size;
				// The fallback path reads asynchronously AFTER this
				// point — invalidate streaming recovery outright so recovery
				// cannot be finalized against re-read data.
				streamRecovery = null;
				streamRecoveryInvalidated = true;
			}
			// TOCTOU guard (cubic review on #78/#79 P1/P2): stat AFTER the
			// read — only when a streaming call is pending or already
			// captured (normal creates skip this syscall entirely). With
			// the async call the capture may still be in flight, so the
			// invalidation flag (not the buffer) is what prevents a
			// stale capture from being used.
			if((streamingCallPending || streamRecovery) && streamStat) {
				var postStat = null;
				try { postStat = fs.statSync(file.name); } catch (e) { /* keep null */ }
				// An unavailable post-stat is an invalidation, not a match
				// (the source may have been replaced) — cubic review on
				// #81 P1.
				if(!postStat ||
				   postStat.size !== streamStat.size ||
				   postStat.mtimeMs !== streamStat.mtimeMs) {
					streamRecovery = null;
					streamRecoveryInvalidated = true;
				}
			}
		...
		// T9 (issue #59): slab capacity = one DATA packet per batch slot
		// (48B header + 8B blockIndex + blockSize body). Grown once per file.
		var pktCap = PAR3_PKT_HDR_SIZE + 8 + blockSize;
		if(HASH_POOL_ENABLED && (!readSlab || readSlab.length < batchSize * pktCap)) {
			readSlab = Buffer.alloc(batchSize * pktCap);
		}
		...
		function readBatch() {
					...
					while(items.length < batchSize && remaining > 0) {
						if(bulkReadOk) {
							...
							var blockOffset = nextBlockIdx * blockSize;
							var buf;
							if(HASH_POOL_ENABLED) {
								var slabOffset = items.length * pktCap + 56;
								fullInputs.copy(readSlab, slabOffset, blockOffset, blockOffset + toRead);
								buf = readSlab.subarray(slabOffset, slabOffset + toRead);
							} else {
								buf = acquireInputBuffer(toRead);
								fullInputs.copy(buf, 0, blockOffset, blockOffset + toRead);
							}
							remaining -= toRead;
							processedBlocks++;
							...
							// Multi-block batched read: read up to batchSize blocks in a single fs.readSync
							// directly into chunkInputs (or fullInputs), eliminating per-block syscall overhead.
							if(isChunkedRecovery && chunkBlockCount === chunkCapacityBlocks) {
								flushChunk();
							}
							var blocksNeeded = batchSize - items.length;
							var chunkSpace = isChunkedRecovery ? (chunkCapacityBlocks - chunkBlockCount) : blocksNeeded;
							var blocksToRead = Math.min(blocksNeeded, Math.ceil(remaining / blockSize), chunkSpace);
							var bytesToRead = Math.min(remaining, blocksToRead * blockSize);

							var targetBuf = isChunkedRecovery ? chunkInputs : fullInputs;
							var targetOffset = isChunkedRecovery ? (chunkBlockCount * blockSize) : (nextBlockIdx * blockSize);

							var bytesRead = 0;
							while(bytesRead < bytesToRead) {
								var n = fs.readSync(fd, targetBuf, targetOffset + bytesRead, bytesToRead - bytesRead, file.size - remaining);
								if(n === 0) break;
								bytesRead += n;
								remaining -= n;
							}
							...
							var blocksReadCount = Math.ceil(bytesRead / blockSize);
							for(var b = 0; b < blocksReadCount; b++) {
								var blockOffset = targetOffset + b * blockSize;
								var actualBytes = Math.min(blockSize, bytesRead - b * blockSize);
								if(actualBytes < blockSize) {
									targetBuf.fill(0, blockOffset + actualBytes, blockOffset + blockSize);
								}

								var buf;
								if(HASH_POOL_ENABLED) {
									var slabOffset = items.length * pktCap + 56;
									targetBuf.copy(readSlab, slabOffset, blockOffset, blockOffset + actualBytes);
									buf = readSlab.subarray(slabOffset, slabOffset + actualBytes);
								} else {
									buf = acquireInputBuffer(actualBytes);
									targetBuf.copy(buf, 0, blockOffset, blockOffset + actualBytes);
								}
								...
								if(isChunkedRecovery) {
									chunkBlockCount++;
									if(chunkBlockCount === chunkCapacityBlocks) {
										flushChunk();
									}
								} else {
									nextBlockIdx++;
				...
				//
				// T9 (issue #59): DATA packets are carved from ONE slab
				// reused across every batch of this file, and block bytes
				// are copied straight into each packet's body window:
				//   targetBuf -> slab[56..]
				// Hash workers receive the body view via structured clone
				// (NO transferList — the transfer would detach the shared
				// slab ArrayBuffer), so the main thread keeps the assembled
				// packet and only the 16-byte hash comes back. This removes
				// the three per-block Buffer.allocs (buf, workerInput,
				// packet) and two of the three per-block copies.
				//
				// Slab reuse across batches is safe because batches are
				// strictly serialized: the next readBatch runs only after
				// pending===0 AND writeBatch has copied every packet into
				// its merged output (the stream never holds slab views).

				var pending = items.length;
				items.forEach(function(item, slot) {
					var bodySize = 8 + item.buf.length;
					var pktLen = PAR3_PKT_HDR_SIZE + bodySize;
					var base = slot * pktCap;
					var pkt = readSlab.subarray(base, base + pktLen);
					fillPacketHeader(pkt, PAR3_PKT_TYPE.DATA, bodySize, self.inputSetId);
					writeUInt64LE(pkt, item.blockIndex, 48);
					var afterChecksum = self._getDataAfterChecksumFor(bodySize);
					_hashBlockParallel(item.blockIndex, item.buf, afterChecksum, function(err, result) {
						var hash;
						if(err) {
							...
							// structured clone deserializes a Buffer sent
							// via postMessage as Uint8Array on the
							// receiving side (Node.js 22+) — Uint8Array
							// has no .copy(). Re-wrap so the inline
							// .copy(pkt, 8, 0, 16) below is valid for
							// both inline-fallback and worker paths.
							hash = Buffer.from(result.hash);
						}
						hash.copy(pkt, 8, 0, 16);
						item.packet = pkt;
						pending--;
				...
				items.forEach(function(item) {
					...
					_hashBlockParallel(item.blockIndex, workerInput, afterChecksum, function(err, result) {
						...
						if(err) {
							...
							// structured clone deserializes a Buffer sent via postMessage as Uint8Array on Node 22+
							hash = Buffer.from(result.hash);
						}
...
function run_par3(files, blockSize, opts, cb) {
	...
	module.exports.fileInfo(files, function(err, info) {
		...
		if(opts && typeof opts.onEvent === 'function') {
			try { opts.onEvent.apply(null, arguments); } catch(e) {}
		}
...
function fileInfo(files, recurse, skipSymlinks, concurrency, cb) {
	...
	scanFiles = function(files, recurse, cbDoneScan) {
		if(!files || files.length === 0) return cbDoneScan(null, []);
		var subResults = new Array(files.length);
		var items = files.map(function(file, idx) {
			return { file: file, idx: idx };
		});
		...
		async.eachLimit(items, concurrency, function(item, cbNextFile) {
			...
			bufs.get(function(buf) {
				...
				async.waterfall([
					...
					function(stat, cb) {
						...
						if(stat.isDirectory()) {
							...
							if(recurse) {
								fs.readdir(file, function(err, dirFiles) {
									...
									}), typeof recurse === 'number' ? recurse - 1 : recurse, function(err, dirRes) {
										if(!err && dirRes) subResults[fileIdx] = dirRes;
										cb(err || true);
									});
								});
							} else {
						...
						if(!stat.isFile()) {
							info = null;
							return cb(new Error(file + ' is not a valid file'));
						}
					...
					if(err && err !== true) return cbNextFile(err);
					cbNextFile(null);
				});
			});
		}, function(err) {
			...
			for(var i = 0; i < subResults.length; i++) {
				var item = subResults[i];
				if(Array.isArray(item)) {
					for(var j = 0; j < item.length; j++) flattened.push(item[j]);
				} else if(item) {
					flattened.push(item);
				}
			}
	...
	scanFiles(files, recurse, function(err, finalResults) {
		bufs.end(function() {
			if(finalResults) {
				results.length = 0;
				for(var i = 0; i < finalResults.length; i++) results.push(finalResults[i]);
			}
			cb(err, results);
		});
	});
...
function par3_create(inputFiles, outputBase, opts, cb) {
	...
	return run_par3(inputFiles, blockSize, {
...
function par3_verify(par3File, cb) {
	...
	var packetCallback = function(type, body, body_length, header) {
		...
		case PAR3_PKT_TYPE.FILE: {
			// Intentionally empty: inputCount comes ONLY from the
			// CAU/MATRIX body (last_input - first_input + 1). FILE
			// packets are per-input-file, not per-block; bumping
			// inputCount here added a phantom block per file (#97).
			break;
			}
...
// B2/C1: pick the coupled RHS backend. The native Gf64Encoder caps G at
// 65536 (src/gf64_addon.cc Gf64Encoder_NAPI_coupled_muladd_arr range
// error); above that use the uncapped JS fallback — the legacy loop had
// no cap either, so repairs must not break at scale.
// PAR3_REPAIR_COUPLED_RHS=0 forces the legacy loop (A/B).
function selectCoupledMuladd(binding, knownCount) {
	if (process.env.PAR3_REPAIR_COUPLED_RHS === '0') return null;
	if (binding && typeof binding.Gf64Encoder === 'function' && knownCount <= 65536) {
		try {
		var enc = new binding.Gf64Encoder(0);
		if (typeof enc.coupled_muladd_arr === 'function') {
			return enc.coupled_muladd_arr.bind(enc);
			}
		} catch (e) {
		/* fall through to the JS fallback */
		}
	}
	return gf64Js.coupled_muladd_arr;
}

// 2D RHS coefficient-matrix budget: n rows x knownCount cols of 8-byte
// coefficients. Bounded by the same 32M-entry cap as the native A matrix
// (repairNativeMatrixAllowed) so a large repair falls back to the coupled
// /legacy paths instead of OOM-ing on the allocUnsafe (cubic review PR #93).
var REPAIR_COEFF_MATRIX_MAX_ENTRIES = 32 * 1024 * 1024;

// Decide whether the fused 2D RHS path is usable for a repair with `n`
// missing equations and `knownCount` available inputs. Mirrors the
// C-side caps (Gf64Encoder caps K and G at 65536) and bounds the
// n x knownCount coefficient matrix by REPAIR_COEFF_MATRIX_MAX_ENTRIES.
function canUseTwoDRhs(binding, n, knownCount) {
	return !!(binding && typeof binding.Gf64Encoder === 'function'
		&& knownCount <= 65536 && n <= 65536
		&& n * knownCount <= REPAIR_COEFF_MATRIX_MAX_ENTRIES
					...
					try {
						...
						var recByIdx = {};
							...
							for(var k = 0; k < repairState.totalInputBlocks; k++) {
								...
								var knownCount = knownInputIndices.length;
								// Coupled / 2D XOR-accumulate:
								// Backend picked by selectCoupledMuladd (native Gf64Encoder,
								// JS gf64_js fallback; G > 65536 and PAR3_REPAIR_COUPLED_RHS=0
								// route to the legacy loop / JS fallback).
								var rhsSlices = new Array(n);
								for(var eq = 0; eq < n; eq++) {
									rhsSlices[eq] = rhsBlocks.slice(eq * blockSize, (eq + 1) * blockSize);
								}
								var knownBuffers = new Array(knownCount);
								for(var gi = 0; gi < knownCount; gi++) knownBuffers[gi] = availableMap[knownInputIndices[gi]];
								// Cauchy coefficient for (known input idx, equation eq).
								// nativeM (when built) is authoritative; the Cauchy fallback
								// mirrors its construction (firstInp/firstRec block numbers).
								var cauchyCoeff = function(inputIdx, eq) {
									if (nativeM) {
										return nativeM.readBigUInt64LE((eq * N_total + inputIdx) * 8);
									}
									var dRow = (BigInt(firstInp) + BigInt(inputIdx)) ^ (BigInt(firstRec) + BigInt(eq));
									return dRow === 0n ? 1n : invert64(dRow);
								};
								// Fill `count` coefficients for equation `eq` into `dest` at
								// byte offset `off * 8`. One implementation shared by the 2D,
								// coupled, and legacy paths (cubic review PR #93 P3).
								var fillCoeffRow = function(dest, off, eq, indices, count) {
									for(var g = 0; g < count; g++) {
										dest.writeBigUInt64LE(cauchyCoeff(indices[g], eq), (off + g) * 8);
									}
								};
								var use2D = canUseTwoDRhs(binding, n, knownCount);
								var coeff_block_2d;
								if (use2D) {
									coeff_block_2d = Buffer.allocUnsafe(n * knownCount * 8);
									for(var eq = 0; eq < n; eq++) {
										fillCoeffRow(coeff_block_2d, eq * knownCount, eq, knownInputIndices, knownCount);
									}
									try {
										var enc2d = new binding.Gf64Encoder(0);
										if (typeof enc2d.two_d_muladd_arr === 'function') {
											enc2d.two_d_muladd_arr(rhsSlices, n, knownBuffers, knownCount, coeff_block_2d, knownCount, numWords);
										} else {
											use2D = false;
										}
									} catch (e) {
								...
								if (!use2D) {
									var coupledFn = selectCoupledMuladd(binding, knownCount);
									var coeffsBuf = Buffer.allocUnsafe(knownCount * 8);
									// Reuse the 2D matrix rows when they were already filled
									// (two_d_muladd_arr missing or threw) instead of refilling.
									var coeffRowFor = function(eq) {
										if (coeff_block_2d) {
											return coeff_block_2d.subarray(eq * knownCount * 8, (eq + 1) * knownCount * 8);
										}
										fillCoeffRow(coeffsBuf, 0, eq, knownInputIndices, knownCount);
										return coeffsBuf;
									};
									if (coupledFn) {
										for(var eq = 0; eq < n; eq++) {
											coupledFn(rhsSlices[eq], knownBuffers, coeffRowFor(eq), numWords, knownCount);
										}
									} else {
										// Legacy per-(known,eq) loop (no coupled kernel / forced A/B).
...
module.exports = {
	...
	// Coupled RHS backend selector (test-facing; used by par3_repair)
	selectCoupledMuladd: selectCoupledMuladd,
	// 2D RHS gate (test-facing; used by par3_repair)
	canUseTwoDRhs: canUseTwoDRhs,
	...
	// T1 (issue #59): deterministic kernel-routing gate (test-facing).
	decideRecoveryKernel: decideRecoveryKernel,
	// Issue #115: adaptive chunk cap calculator (test-facing).
	decideChunkCapBytes: decideChunkCapBytes,
	// Issue #119: topology discovery and optimal chunk cap helpers
	getCpuTopology: getCpuTopology,
	resetCpuTopologyCache: resetCpuTopologyCache,
	getOptimalChunkCap: getOptimalChunkCap
};