...
function _hashBlockParallel(blockIndex, blockData, afterChecksum, cb) {
	...
	if(found) {
		...
		found.cb = cb;
		try {
			// cubic P2: copy slab-backed subarray views to a fresh Buffer
			// before transferring; transferable views must not alias the
			// per-batch slab ArrayBuffer (which we keep while the worker
			// runs). Also copy so views from user-supplied Buffers that
			// aren't transfer-listed elsewhere don't trip structured-clone.
			var transferBuf = Buffer.isBuffer(blockData)
				? blockData
