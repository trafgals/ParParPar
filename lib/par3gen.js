	...
	global.__par3ProfStart('setup');
		...
		function readBatch() {
					...
					while(items.length < batchSize && remaining > 0) {
						...
						var actualBytes = bulkFileBuf ? toRead : bytesRead;
						if(isChunkedRecovery) {
							// Cubic review P2: copy actualBytes and zero-fill ONLY the partial tail on short reads
							var dstOffset = chunkBlockCount * blockSize;
							buf.copy(chunkInputs, dstOffset, 0, actualBytes);
							if (actualBytes < blockSize) {
								chunkInputs.fill(0, dstOffset + actualBytes, dstOffset + blockSize);
							}
							chunkBlockCount++;
							...
							nextBlockIdx++;
							buf.copy(fullInputs, writeOffset, 0, actualBytes);
							if (actualBytes < blockSize) {
								fullInputs.fill(0, writeOffset + actualBytes, writeOffset + blockSize);
							}
						}

						// Cubic review P2: truncate buf to actualBytes if short read so DATA packet matches recovery
						if(buf.length > actualBytes) {
							buf = buf.subarray(0, actualBytes);
						}
