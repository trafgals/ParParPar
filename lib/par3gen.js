						...
						try {
							...
							var recByIdx = {};
								...
								for(var k = 0; k < repairState.totalInputBlocks; k++) {
									...
									// Native via Gf64Encoder when available, else the JS
									// gf64_js fallback; PAR3_REPAIR_COUPLED_RHS=0 forces the
									// legacy per-(known,eq) loop (A/B).
									var enc = null;
									if (binding && typeof binding.Gf64Encoder === 'function') {
										try { enc = new binding.Gf64Encoder(0); } catch (e) { enc = null; }
									}
									var coupledFn = (enc && typeof enc.coupled_muladd_arr === 'function'
									                 && process.env.PAR3_REPAIR_COUPLED_RHS !== '0')
										? enc.coupled_muladd_arr.bind(enc)
										: (process.env.PAR3_REPAIR_COUPLED_RHS !== '0' ? gf64Js.coupled_muladd_arr : null);
