					...
					try {
						...
						var recByIdx = {};
							...
							for(var k = 0; k < repairState.totalInputBlocks; k++) {
								...
								// Coupled XOR-accumulate: one call per equation row
								// (rhs[eq] ^= sum_known coeff(eq,k) * in[k]) replaces
								// knownCount x n mul_arr calls plus the per-byte JS XOR.
								...
								if (coupledFn) {
									for(var eq = 0; eq < n; eq++) {
										...
										coupledFn(rhsBlocks.slice(eq * blockSize, (eq + 1) * blockSize), knownBuffers, coeffsBuf, numWords, knownCount);
									}
								} else {
