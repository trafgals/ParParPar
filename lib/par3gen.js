				...
				if(useStreamingCreate && file.name) {
					...
					try {
						...
						bindingRef.par3_create_streaming(file.name, {
							...
							// Anything else keeps the legacy compute as the source
							// of truth (the streaming call stays telemetry-only).
							if(!streamErr && streamResult && streamResult.recoveryBuffer
