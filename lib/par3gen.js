...
run: function(eventCb, completeCb) {
	...
	global.__par3ProfStart('setup');
		...
		//
		// Adaptive chunk sizing (Issue #115):
		...
		if (maxMemoryBudget > 0) {
			chunkCapBytes = Math.min(chunkCapBytes, Math.max(blockSize, Math.floor(maxMemoryBudget / 2)));
		}
		...
		if(isChunkedRecovery) {
			if (maxMemoryBudget > 0) {
				// Reserve accumulator bytes (and Fenger scratch if Fenger is chosen)
				// from memory budget so combined allocation stays within limit
				var isFenger = (routingDecision.kernel === 'fenger');
				var recoveryBufferReserve = (isFenger ? 2 : 1) * numRecovery * blockSize;
				var availableInputBudget = Math.max(blockSize, maxMemoryBudget - recoveryBufferReserve);
				chunkCapacityBlocks = Math.max(1, Math.floor(Math.min(chunkCapBytes, availableInputBudget) / blockSize));
			} else {
