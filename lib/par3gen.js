...
PAR3Gen.prototype = {
	...
	_createStartPacket: function() {
		// PAR3 spec: parent InputSetID(8) + unique(16) + blockSize(4) + blockCount(8) + gfSize(2) + generator
		var bodySize = 34;
		var header = createPacketHeader(PAR3_PKT_TYPE.START, bodySize, this.inputSetId);

		header.writeBigUInt64LE(BigInt(this.opts.blockSize), 64);
...
run: function(eventCb, completeCb) {
		...
		function ensureAccumulator() {
			...
			accumulator = Buffer.alloc(Math.min(totalRecoverySize, MAX_IN_MEMORY));
		}
					...
					while(true) {
						...
						if(packet) {
							if(!self._writePacket(packet)) {
								self._outputStream.once('drain', readBlockFull);
								return;
							}
						}
					...
					fs.close(fd, function() {
						...
						try {
							...
							var hasNativeSolve = binding && typeof binding.solve_and_reconstruct === 'function';
								...
								// Cauchy RHS adjustment for partial-loss repair.
								// recovery[k] = sum_{j all} coeff[k][j]*data[j], but only SOME
								// input blocks are available; the solver reconstructs the missing ones.
								// We must subtract the known-block contribution (XOR in GF(2)) before
								// passing rhsBlocks to solve_and_reconstruct. Modeled on
								// test/par3-repair-parity.js:348-377 (canonical reference).
								var lenWords = blockSize / 8;
								var coeffBuf = Buffer.alloc(8);
								var tmpBuf = Buffer.alloc(blockSize);
								for(var eq = 0; eq < n; eq++) {
									var rhsRowOff = eq * blockSize;
									for(var di = 0; di < repairState.dataBlocks.length; di++) {
										var dataBlock = repairState.dataBlocks[di];
										var inpIdx = dataBlock.block_index;
										var xi = BigInt(firstRec + eq);
										var yj = BigInt(firstInp + inpIdx);
										var denom = xi ^ yj;
										var coeff = denom === 0n ? 0n : invert64(denom);
										coeffBuf.writeBigUInt64LE(coeff, 0);
										tmpBuf.fill(0);
										gf64Js.mul_arr(tmpBuf, dataBlock.data, coeffBuf, lenWords, 1);
										for(var b = 0; b < blockSize; b++) {
											rhsBlocks[rhsRowOff + b] ^= tmpBuf[b];
										}
									}
								}
								...
								if(hasNativeSolve) {
									...
									// JS kernel returns 0 on success, -1 on singular; normalize
									// to bool so `!ok` below doesn't misread 0 as failure.
									ok = gf64Js.solve_and_reconstruct(A, rhsBlocks, n, blockSize, 0) === 0;
								}
								if(!ok) {
