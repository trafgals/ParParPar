...
function _ensureInputPool(totalSize) {
	...
	var poolSize = Math.max(64 * 1024 * 1024, totalSize + 64 * 1024);
...
run: function(eventCb, completeCb) {
	...
	global.__par3ProfStart('setup');
		...
		if (gateAllowsMatvec && !useStreamingCreate && bindingRef && typeof bindingRef.build_coefficient_matrix === 'function'
		...
		var processFileFull = function(idx) {
			...
			try {
				...
				if(fileStat.size >= 0x100000000) throw new Error('bulk-read-skip-large-file');
