	...
	global.__par3ProfStart('setup');
		...
		function readBatch() {
					...
					while(items.length < batchSize && remaining > 0) {
						...
						if(isChunkedRecovery) {
							// Cubic review P1: fill tail with zero and copy only actualBytes
							var dstOffset = chunkBlockCount * blockSize;
							chunkInputs.fill(0, dstOffset, dstOffset + blockSize);
							...
							nextBlockIdx++;
							fullInputs.fill(0, writeOffset, writeOffset + blockSize);
