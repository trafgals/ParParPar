...
// K5 (issue #46): the Fenger engine now pads non-power-of-2 counts up to
// the next power of 2 internally (bit-exact via the Fenger identity), so
// eligibility no longer requires power-of-2. We bound the padding cost
// instead: reject counts whose next power of 2 more than doubles them
// (padding factor <= 2x), which keeps the tree size within 2x the real
// workload — the 10G/100k target (2,621,440 / 100,000) pads to 2^22 /
// 2^17, both < 2x.
...
function dispatchRecovery(binding, inputs, outputs, numInputs, numRecovery, blockSize, firstInput, firstRecovery, numThreads, fallbackFn) {
	...
	// K5: engine pads non-power-of-2 internally; only the padding-cost
	// bound (<= 2x) and blockSize%8 gate apply.
	var fengerEligible = hasFenger
		&& _fengerPaddingReasonable(numInputs)
		&& _fengerPaddingReasonable(numRecovery)
		&& blockSize % 8 === 0
