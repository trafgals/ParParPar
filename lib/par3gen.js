	...
	global.__par3ProfStart('setup');
		...
		if(isChunkedRecovery) {
			if (isSimulatedCap) {
				...
				// Cubic review P1: reserve accumulator bytes from memory budget so combined allocation stays within limit
				var accumulatorBytes = numRecovery * blockSize;
				var availableInputBudget = Math.max(blockSize, maxMemoryBudget - accumulatorBytes);
