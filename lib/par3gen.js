	...
	global.__par3ProfStart('setup');
		...
		if(useComputeRecoveryFull) {
			...
			var processFileFull = function(idx) {
				...
				// TOCTOU guard (cubic review on #78 P1/P2): stat AFTER the
				// read regardless of which read path ran (the bulk-read
				// fstat compares two pre-read stats and the catch fallback
				// never assigns it). A source that changed between the
				// streaming read and the JS data read invalidates the
				// streaming recovery — recovery must match the data packets.
				var postStat = null;
				try { postStat = fs.statSync(file.name); } catch (e) { /* keep null */ }
				if(streamRecovery && streamStat && postStat &&
				   (postStat.size !== streamStat.size || postStat.mtimeMs !== streamStat.mtimeMs)) {
					streamRecovery = null;
				}
				readBlockFull();
