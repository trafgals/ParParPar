	...
	global.__par3ProfStart('setup');
		...
		function flushChunk() {
			...
			if (routingDecision.kernel === 'barycentric' && typeof bindingRef.compute_recovery_barycentric === 'function') {
				...
				if (typeof bindingRef.xor_buffers === 'function') {
					bindingRef.xor_buffers(accumulator, fengerChunkScratch);
				} else {
					for (var b = 0; b < accumulator.length; b += 8) {
						accumulator.writeBigUInt64LE(
							accumulator.readBigUInt64LE(b) ^ fengerChunkScratch.readBigUInt64LE(b),
							b
						);
					}
				}
