...
run: function(eventCb, completeCb) {
	...
	global.__par3ProfStart('setup');
		...
		// (default 64 MiB) or PAR3_FORCE_CHUNKED=1, stream inputs in bounded chunks
		// using compute_recovery_accumulate/barycentric/fenger, eliminating the fullInputs buffer
		// and keeping peak RSS <= 300 MiB across all archive shapes (up to 64+ GiB).
		var chunkCapBytes = 64 * 1024 * 1024; // 64 MiB default
