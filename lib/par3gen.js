...
// Scales chunk buffer capacity based on total archive size:
// - Shapes < 4 GiB: 64 MiB default keeps memory footprint minimal (< 300 MiB).
// - Shapes 4 GiB .. 16 GiB: 128 MiB reduces chunk cycle overhead for mid-size archives.
// - Shapes >= 16 GiB: 256 MiB reduces chunk cycles by 4x (e.g. 512 -> 128 flushes on 32 GiB),
//   recovering ~34 MB/s throughput while keeping peak RSS safely < 450 MiB.
// Total buffer allocation (chunkInputs + recovery + optional Fenger scratch)
// must stay within target budget (default 384 MiB buffer budget, keeping peak RSS < 450 MiB).
function decideChunkCapBytes(totalInputBytes, numRecovery, blockSize, kernel, memoryLimit) {
	var chunkCapBytes = 64 * 1024 * 1024; // 64 MiB default for < 4 GiB
	if (totalInputBytes >= 16 * 1024 * 1024 * 1024) {
		chunkCapBytes = 256 * 1024 * 1024;
	} else if (totalInputBytes >= 4 * 1024 * 1024 * 1024) {
		chunkCapBytes = 128 * 1024 * 1024;
	}
	...
	if (maxMemoryBudget > 0) {
		var availBudget = maxMemoryBudget - recoveryBufferReserve;
		if (!hasExplicitChunkCap && availBudget < minCap) {
			// Cubic review P2: When recovery reserve consumes the memory budget,
	...
	} else if (!hasExplicitChunkCap) {
		var availDefaultBudget = defaultBufferBudget - recoveryBufferReserve;
		if (availDefaultBudget < minCap) {
			// Cubic review P2: When recovery reserve consumes the 384 MiB default budget,
			// chunked recovery cannot fit within the <450 MiB RSS target.
			return 0;
		}
		chunkCapBytes = Math.min(chunkCapBytes, availDefaultBudget);
	}
...
run: function(eventCb, completeCb) {
	...
	global.__par3ProfStart('setup');
		...
		if(computeRecoveryFullBinding && typeof computeRecoveryFullBinding.compute_recovery_full === 'function') {
			...
			// Cubic review P2: Route workloads where recovery reserve consumes the budget
			// through the spill/per-batch path instead of forcing an allocation that cannot fit.
			if (hasExplicitChunkCap || (recoveryReserve + blockSize <= targetMemoryBudget)) {
