...
run: function(eventCb, completeCb) {
	...
	global.__par3ProfStart('setup');
		...
		if(isChunkedRecovery) {
			chunkCapacityBlocks = Math.max(1, Math.floor(maxMemoryBudget / blockSize));
					...
					while(items.length < batchSize && remaining > 0) {
						...
						if(isChunkedRecovery) {
							if(buf.length < blockSize) {
								...
								buf.copy(padded);
								padded.copy(chunkInputs, chunkBlockCount * blockSize);
							} else {
								buf.copy(chunkInputs, chunkBlockCount * blockSize);
							}
							chunkBlockCount++;
