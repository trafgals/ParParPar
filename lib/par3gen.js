					...
					try {
						...
						var recByIdx = {};
							...
							for(var k = 0; k < repairState.totalInputBlocks; k++) {
								...
								var use2D = (binding && typeof binding.Gf64Encoder === 'function' && knownCount <= 65536 && n <= 65536 && process.env.PAR3_REPAIR_COUPLED_RHS !== '0');
								if (use2D) {
									var coeff_block_2d = Buffer.allocUnsafe(n * knownCount * 8);
									for(var eq = 0; eq < n; eq++) {
										if (nativeM) {
											var rowBase = eq * N_total;
											for(var g = 0; g < knownCount; g++) {
												coeff_block_2d.writeBigUInt64LE(nativeM.readBigUInt64LE((rowBase + knownInputIndices[g]) * 8), (eq * knownCount + g) * 8);
											}
										} else {
											var yRow = BigInt(firstRec) + BigInt(eq);
											for(var g = 0; g < knownCount; g++) {
												var dRow = (BigInt(firstInp) + BigInt(knownInputIndices[g])) ^ yRow;
												coeff_block_2d.writeBigUInt64LE(dRow === 0n ? 1n : invert64(dRow), (eq * knownCount + g) * 8);
											}
										}
									}
									try {
									...
									if (coupledFn) {
										for(var eq = 0; eq < n; eq++) {
											if (nativeM) {
												var rowBase = eq * N_total;
												for(var g = 0; g < knownCount; g++) {
													coeffsBuf.writeBigUInt64LE(nativeM.readBigUInt64LE((rowBase + knownInputIndices[g]) * 8), g * 8);
												}
											} else {
												var yRow = BigInt(firstRec) + BigInt(eq);
												for(var g = 0; g < knownCount; g++) {
													var dRow = (BigInt(firstInp) + BigInt(knownInputIndices[g])) ^ yRow;
													coeffsBuf.writeBigUInt64LE(dRow === 0n ? 1n : invert64(dRow), g * 8);
												}
											}
											coupledFn(rhsSlices[eq], knownBuffers, coeffsBuf, numWords, knownCount);
										}
									} else {
										...
										for(var ai = 0; ai < knownCount; ai++) {
											...
											for(var eq2 = 0; eq2 < n; eq2++) {
												var coeff;
												if (nativeM) {
													coeff = nativeM.readBigUInt64LE((eq2 * N_total + inputIdx) * 8);
												} else {
													var xi = BigInt(firstInp + inputIdx);
													var yk = BigInt(firstRec + eq2);
													var denom = xi ^ yk;
