...
// Picks compute_recovery_barycentric for large workloads; falls back to
// the legacy kernel otherwise.
//
// Note on cost: the Barycentric kernel is asymptotically O(N*R*B) — the
// same as the legacy 2D-muladd path. The plan originally described it as
// "near-linear in N", but that requires a future FFT polynomial-multiply
// primitive. PR-1 of T8 establishes the call shape (gf64_poly_mul_padded);
// the actual multiply primitive is deferred research-grade work (Gao-Mateer
// tower-of-extensions or subfield NTT). The kernel today is a cleaned-up
// direct Cauchy formula with the per-row Itoh-Tsujii batched inversion
// and a pre-computed x_c[] array (PR-4 micro-opt).
//
// PR-7 DECISION: keep the default at 10000.
//
// The original PR-7 of the T8 plan lowered the default to ~1000 once the
// Bostan-Schost MPE landed. That decision was contingent on the kernel
// being asymptotically faster than legacy at the smaller dispatch range —
// which is NOT the case. Both kernels share the same O(N*R*B) cost; the
// Barycentric kernel only wins on the per-row inversion + per-row muladd
// ordering (batched Itoh-Tsujii inversion + AVX-512 region_muladd_arr)
// and the per-call x_c[] hoist from PR-4, which is a constant-factor
// win not a threshold-shifting one.
//
// The 10000 threshold remains a measured crossover where this path matches
// or beats the legacy 2D-muladd path on the canonical 1 GiB / 10K
// workload — see test/bench/par3-cauchy-fft-bench.js. Lowering it
// without the FFT-multiply primitive would just shift compute onto the
// less-tuned dispatch (the legacy 2D-muladd path) for smaller workloads
// without a measurable gain. Once the FFT-multiply lands and the
// asymptotic gap opens, this default will be revisited.
//
// Threshold: PAR3_BF64_MIN_INPUTS env (default 10000). Requires
// blockSize%8==0 for the AVX-512 / AVX-2 region_muladd_arr dispatch.
// Binding is the first arg so the test layer can inject a mock. The
// optional fallbackFn lets non-single-call sites (per-batch path) point
// at a kernel whose output buffer matches their XOR-into-accumulator
// semantics.
	...
	_processRecoveryBatch: function(batch, firstInputIdx, firstRecovery, numRecovery, accumulator) {
		...
		global.__par3ProfStart('kernel');
			...
			if(numRecoveryThisCall < numRecovery) {
				...
				var nextFirstRecovery = firstRecoveryThisChunk + BigInt(numRecoveryThisCall);
	...
	global.__par3ProfStart('setup');
		...
		if(useComputeRecoveryFull) {
			...
			var finalizeRecovery = function() {
				...
				// v2-4: if the matrix-build worker hasn't finished by now, the
				// file read took longer than the matrix build, so we wait
				// here. Either way, this is where the parallelism converges.
				// Use Atomics.wait on a SharedArrayBuffer for a true blocking
				// wait that doesn't burn CPU.
				if (!precomputedCoeffDone) {
					var sab = new SharedArrayBuffer(4);
					var i32 = new Int32Array(sab);
					// Hook the worker's message handler to also notify us via the SAB.
					// (Re-attach a listener that calls Atomics.notify.)
					coeffWorker.once('message', function() {
