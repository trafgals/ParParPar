	...
	global.__par3ProfStart('setup');
		...
		if(useComputeRecoveryFull) {
			...
			var processFileFull = function(idx) {
				...
				// TOCTOU guard: if the source changed between the streaming
				// read and the JS data read, the streaming recovery no longer
				// matches the data packets — discard it and let the legacy
				// compute produce the recovery from the JS-read data.
				if(streamRecovery && fileStat && streamStat &&
				   (fileStat.size !== streamStat.size || fileStat.mtimeMs !== streamStat.mtimeMs)) {
					streamRecovery = null;
				}
				readBlockFull();
