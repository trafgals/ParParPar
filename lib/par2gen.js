...
var pathToPar2 = utils.pathToPar;
...
function _calcNumSlicesCommon(s, baseValue, fileValues) {
	if(s.unit == 'power' || s.unit == 'log' || s.unit == 'ilog') {
		if(baseValue == 0) return 0;
		switch(s.unit) {
			case 'power':
				return Math.pow(baseValue, s.value);
			case 'log':
				var val = Math.log(baseValue) / Math.log(s.value);
				if(!isFinite(val) || isNaN(val)) throw new RangeError('Invalid logarithm value');
				return val;
			case 'ilog':
				baseValue *= s.value;
				return (baseValue / Math.max(Math.log(baseValue)/Math.log(2), 1));
		}
	}
	if(fileValues && (s.unit == 'largest_files' || s.unit == 'smallest_files')) {
		var sorted = fileValues.sort(s.unit == 'largest_files' ? function(a, b) {
			return b-a;
		} : function(a, b) {
			return a-b;
		});
		if(s.unit == 'smallest_files') {
			// remove 0 value files
			var i = 0;
...
function calcNumInputSlices(spec, files, totalSize, multiple) {
	if(typeof spec === 'number') return spec; // size/count shortcut
	if(spec.value < 0) throw new RangeError('Input slice value cannot be negative');
	
	var scale = spec.scale || 1;
	if(spec.unit == 'count') {
		return Math.round(scale * -spec.value);
	}
	if(spec.unit == 'bytes' && scale == 1) { // if scale == 1, don't auto adjust the value after scaling
		return scale * spec.value;
	}
	var amount = scale * _calcNumSlicesCommon(spec, totalSize, files && files.map(function(file) {
		return file.size;
	}));
	if(amount < 1) amount = 1;
	if(amount % multiple != 0) {
		amount = Math.ceil(amount / multiple) * multiple;
	}
	return amount;
}
...
function calcNumRecoverySlices(spec, sliceSize, inSlices, files) {
	...
	return spec.map(function(s) {
		...
		var amount = _calcNumSlicesCommon(s, inSlices, files && files.map(function(file) {
...
function PAR2Gen(fileInfo, sliceSize, opts) {
	...
	var o = this.opts = {
		...
		creator: 'ParPar (library) v' + require('../package').version + ' [https://github.com/animetosho/parpar]',
	...
	// handle complex sliceSize definitions
	var minSliceSize = o.minSliceSize, maxSliceSize = o.maxSliceSize;
	var ssUnit, adjSSmin = false, adjSSmax = false;
	if(typeof sliceSize == 'object') {
		ssUnit = sliceSize.unit;
		sliceSize = calcNumInputSlices(sliceSize, fileInfo, totalSize, o.sliceSizeMultiple);
	} else {
		ssUnit = sliceSize < 0 ? 'count' : 'bytes';
	}
	...
	if(minSliceSize !== null) {
		var sameUnitMinSliceSize = false;
		if(typeof minSliceSize == 'object') {
			sameUnitMinSliceSize = ssUnit == minSliceSize.unit;
			minSliceSize = calcNumInputSlices(minSliceSize, fileInfo, totalSize, o.sliceSizeMultiple);
		} else
			sameUnitMinSliceSize = (minSliceSize <= 0 ? 'count' : 'bytes') == ssUnit;
		if((sliceSize < 0) == (minSliceSize <= 0) && Math.abs(sliceSize) < Math.abs(minSliceSize)) {
			if(sameUnitMinSliceSize)
	...
	if(maxSliceSize !== null) {
		var sameUnitMaxSliceSize = false;
		if(typeof maxSliceSize == 'object') {
			sameUnitMaxSliceSize = ssUnit == maxSliceSize.unit;
			maxSliceSize = calcNumInputSlices(maxSliceSize, fileInfo, totalSize, o.sliceSizeMultiple);
		} else
			sameUnitMaxSliceSize = (maxSliceSize <= 0 ? 'count' : 'bytes') == ssUnit;
		if((sliceSize < 0) == (maxSliceSize <= 0) && Math.abs(sliceSize) > Math.abs(maxSliceSize)) {
			if(sameUnitMaxSliceSize)
	...
	if(minSliceSize !== null && maxSliceSize !== null) {
		if((minSliceSize <= 0) == (maxSliceSize <= 0) && Math.abs(maxSliceSize) < Math.abs(minSliceSize)) {
			if(minSliceSize <= 0)
				throw new Error('Specified min/max slice count range is invalid or cannot be satisfied: ' + (-minSliceSize) + ' (min) > ' + (-maxSliceSize) + ' (max)');
			else
				throw new Error('Specified min/max slice size range is invalid or cannot be satisfied: ' + friendlySize(minSliceSize) + ' (min) > ' + friendlySize(maxSliceSize) + ' (max)');
		}
	}
	if(minSliceSize === null) minSliceSize = sliceSize;
	else if(adjSSmin) sliceSize = minSliceSize;
	if(maxSliceSize === null) maxSliceSize = sliceSize;
	else if(adjSSmax) sliceSize = maxSliceSize;
	...
	if(o.recoverySlices < 1) // no recovery generated => ignore memory limit
		o.memoryLimit = 0;
	...
	if(o.cpuMinChunkSize < o.sliceSize && o.openclDevices.length && o.recoverySlices > 0) {
		...
		o.openclDevices.forEach(function(oclDev) {
			...
			if(oclDev.memoryLimit === null) {
				oclDev.memoryLimit = Math.max(Math.floor(devInfo.memory_global * 0.8), oclDev.minChunkSize, 8*1048576);
				if(is64bPlatform)
					// hard limit default to 4GB
					oclDev.memoryLimit = Math.min(4096*1048576, oclDev.memoryLimit);
				else
					// be safer on 32b platforms
					oclDev.memoryLimit = Math.min(512*1048576, oclDev.memoryLimit);
			}
		...
		o.openclDevices.forEach(function(oclDev) {
			...
			var devScaledMemLimit = Math.floor(oclDev.memoryLimit / oclDev.ratio);
			minMemoryLimit = Math.min(minMemoryLimit, devScaledMemLimit);
		});
		if(minMemoryLimit == Number.MAX_VALUE) minMemoryLimit = 0;
	}
	
	// number of chunk sized buffers needed to pass data to/from backend
	this.procInStagingBufferCount = o.processBatchSize*stagingCount;
	...
	if(o.memoryLimit) {
		...
		if(o.minChunkSize && (o.recDataSize+1) * o.minChunkSize > o.memoryLimit)
			throw new Error('Cannot accommodate target memory limit (' + friendlySize(o.memoryLimit) + ') with target minimum chunk size (' + friendlySize(o.minChunkSize) + '). At least ' + (o.recDataSize+1) + ' buffers of the minimum chunk size need to be allocated.');
		if(cpuMinChunk * (this.procInStagingBufferCount+1) > o.memoryLimit)
			throw new Error('Cannot accommodate target memory limit (' + friendlySize(o.memoryLimit) + ') with target minimum CPU chunk size (' + friendlySize(cpuMinChunk) + '). At least ' + (this.procInStagingBufferCount+1) + ' buffers of the minimum chunk size need to be allocated.');
	}
	...
	var reqMem = o.sliceSize * o.recoverySlices + Math.max(o.recDataSize * o.sliceSize, this.procInStagingBufferCount * Math.ceil(o.sliceSize*cpuRatio/2)*2);
	this.passes = 1;
	this.chunks = 1;
	...
	if(passes > 1 && o.recoverySlices > 0) {
		// TODO: this should probably be checked against each OpenCL device
		...
		if(chunkSize < minChunkSize) {
			...
			this.slicesPerPass = Math.floor(minMemoryLimit / chunkSize);
			// check limits on the CPU side, after adding transfer memory
			if(o.memoryLimit) {
				var overhead = Math.max(this.procInStagingBufferCount*Math.ceil(chunkSize*cpuRatio/2)*2, o.recDataSize*chunkSize);
				var cpuSlicesPerPass = Math.floor((o.memoryLimit-overhead) / chunkSize);
				if(cpuSlicesPerPass < 1)
					throw new Error('Cannot accommodate memory limit (' + friendlySize(o.memoryLimit) + '); a chunk size of ' + friendlySize(chunkSize) + ' was chosen, but a minimum of 1 recovery + ' + friendlySize(overhead) + ' processing buffer needs to be held in memory');
				this.slicesPerPass = Math.min(this.slicesPerPass, cpuSlicesPerPass);
			}
			if(this.slicesPerPass < 1)
				throw new Error('Cannot accommodate target memory limit (' + friendlySize(minMemoryLimit) + '); a minimum of 1 chunk of size ' + friendlySize(chunkSize) + ' needs to be held in memory');
	...
	else if(o.recoverySlices < 1)
		// no recovery - just use the requested read size
		this.readSize = o.seqReadSize;
	else {
...
PAR2Gen.prototype = {
	...
	_initOutputFiles: function(cb) {
		...
		async.eachSeries(this.recoveryFiles, function(rf, cb) {
			...
			rf.open(self.opts.outputOverwrite, function(err) {
				if(err) return cb(err);
				// TODO: may wish to be careful that prealloc doesn't screw with the reading I/O
				// TODO: perhaps give user option on prealloc
	...
	writeFile: function(rf, cbProgress, cb) {
		var cPos = 0;
		...
		async.timesSeries(rf.packets.length, function(pktI, cb) {
			...
			async.series([
				function(cb) {
					// TODO: this grabs at most one recovery packet at a time, limiting writev potential, as we may have more available
				...
				rf.open.bind(rf, self.opts.outputOverwrite),
				function(cb) {
					if(cbProgress) cbProgress('writing_file_pos', rf, cPos + pkt.dataChunkOffset);
					rf.writePackets(pktI, cPos, cb);
				}
			], function(err) {
				if(err && err !== true)
					return cb(err);
	...
	writeFiles: function(cbProgress, cb) {
		async.eachSeries(this.recoveryFiles, function(rf, cb) {
			if(cbProgress) cbProgress('writing_file', rf);
			this.writeFile(rf, cbProgress, cb);
		}.bind(this), cb);
	},
	closeFiles: function(cb) {
		var sync = this.opts.outputSync;
		async.eachSeries(this.recoveryFiles, function(rf, cb) {
			rf.close(sync, cb);
		}, cb);
	},
	...
	_readPass: function(chunkSize, cbProgress, cb) {
		...
		if(seeking) {
			...
			reader.run(function(err, data) {
				...
				if(self.opts.recoverySlices < 1) {
					if(cbProgress) {
						var slicePosTo = (data.pos+data.buffer.length) / self.opts.sliceSize;
						var slicePosToBase = Math.floor(slicePosTo);
						for(var slicePos = Math.floor(data.pos / self.opts.sliceSize); slicePos < slicePosToBase; slicePos++)
							cbProgress('processing_slice', data.file, slicePos);
						cbProgress('processing_slice_part', slicePosTo-slicePosToBase);
					}
					return data.release();
				}
	...
	// returns the number of chunks to process on the current chunk_pass
	numChunksThisChunkPass: function() {
		var self = this;
		return this.files.reduce(function(count, file) {
			if(file.size == 0 || file.size <= self.chunkOffset) return count;
			return count + Math.ceil((file.size - self.chunkOffset) / self.opts.sliceSize);
		}, 0);
	},
	// computes the total number of input chunks to process
	totalNumInputChunks: function() {
		var self = this;
		return this.files.reduce(function(count, file) {
			if(file.size > 0) {
				// add number of chunks, excluding the final slice
				var nonLastSlices = file.numSlices-1;
				count += nonLastSlices * self.chunks;
				// for the final slice, compute the number of chunks
				var lastSliceSize = file.size - nonLastSlices*self.opts.sliceSize;
				count += Math.ceil(lastSliceSize / self._chunkSize);
			}
			return count;
		}, 0);
	},
	...
	runChunkPass: function(cbProgress, cb) {
		...
		if(cbProgress) cbProgress('begin_chunk_pass', self.passNum, self.passChunkNum);
		...
		async.series([
			...
			function(cb) {
				...
				if(cbProgress) cbProgress('chunk_pass_write', self.passNum, self.passChunkNum);
				self.finish(cb);
			},
			this.writeFiles.bind(this, cbProgress),
			function(cb) {
				if(cbProgress) cbProgress('chunk_pass_complete', self.passNum, self.passChunkNum);
				self.passChunkNum++;
	...
	runPass: function(cbProgress, cb) {
		...
		if(cbProgress) cbProgress('begin_pass', self.passNum);
		this._setSlices(this.sliceOffset);
		
		async.whilst(function(cb) {
			var result = self.chunkOffset < self.opts.sliceSize;
			if(async.cargoQueue) cb(null, result); // async 3.x
			else return result; // async 1.x, 2.x
		}, this.runChunkPass.bind(this, cbProgress), function(err) {
			if(err) return cb(err);
			self.finishPass(cbProgress, cb);
		});
	},
	finishPass: function(cbProgress, cb) {
		var self = this;
		(function(cb) {
			if(!self._chunker) return cb();
			if(cbProgress) cbProgress('pass_write', self.passNum);
			// write chunk headers
			...
			async.series(
				...
				function() {
					self.writeFiles(null, cb);
				}
			);
		})(function() {
			if(cbProgress) cbProgress('pass_complete', self.passNum);
	...
	run: function(cbProgress, cb) {
		...
		async.whilst(function(cb){
			...
			var result = self.sliceOffset < self.opts.recoverySlices || (self.passNum == 0 && self.passChunkNum == 0);
			if(async.cargoQueue) cb(null, result); // async 3.x
			else return result; // async 1.x, 2.x
			...
			if(cbProgress) cbProgress('closing_files');
...
module.exports = {
	...
	fileInfo: function(files, recurse, skipSymlinks, concurrency, cb) {
		return require('./filescan').fileScan(files, recurse, skipSymlinks, concurrency, 'md5', cb);
	},
