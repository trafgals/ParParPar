...
function coupled_muladd_arr(out, in_blocks, coeff_buf, len_words, G) {
	for (var g = 0; g < G; g++) {
		var c = coeff_buf.readBigUInt64LE(g * 8);
		if (c === 0n) continue;
		var in_buf = in_blocks[g];
		for (var i = 0; i < len_words; i++) {
			var w = in_buf.readBigUInt64LE(i * 8);
			if (w === 0n) continue;
			var prev = out.readBigUInt64LE(i * 8);
			out.writeBigUInt64LE(prev ^ gf64_mul(w, c), i * 8);
		}
	}
}
