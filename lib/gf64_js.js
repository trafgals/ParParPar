...
// Coupled multiply-XOR-accumulate (matches gf64_region_coupled_muladd_*):
//   out[w] ^= sum_g in_blocks[g][w] * coeff_buf[g]
// One call replaces G mul_arr calls plus a JS XOR loop in the repair RHS.
function coupled_muladd_arr(out, in_blocks, coeff_buf, len_words, G) {
	var coeffs = new Array(G);
	for (var g = 0; g < G; g++) {
		coeffs[g] = coeff_buf.readBigUInt64LE(g * 8);
	}
	for (var i = 0; i < len_words; i++) {
		var acc = 0n;
		for (var g2 = 0; g2 < G; g2++) {
			acc ^= gf64_mul(in_blocks[g2].readBigUInt64LE(i * 8), coeffs[g2]);
		}
		out.writeBigUInt64LE(out.readBigUInt64LE(i * 8) ^ acc, i * 8);
	}
}
...
module.exports = {
	...
	mul: mul,
	mul_arr: mul_arr,
	coupled_muladd_arr: coupled_muladd_arr,
