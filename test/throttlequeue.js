...
testAsync('natural timeout should call callback with cancelled=false and empty queue', function(done) {
	var dt = new DelayThrottle(10);
	var fired = false;
	dt.pass(1, function(cancelled) {
		try {
			fired = true;
			assert.strictEqual(cancelled, false, 'Expected cancelled === false on natural timer expiration');
			assert.deepStrictEqual(dt.queue, {}, 'Queue should be empty after timeout');
			done();
		} catch(e) {
			done(e);
		}
