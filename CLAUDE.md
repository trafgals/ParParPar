...
- **Branch-naming convention:** the PA/PB/PC/PD/PE task prefixes are commit-subject conventions, NOT branch prefixes. There are two valid branch styles:
  - **Long-lived integration branches** (named `vN/<topic>`): receive sequential commits against Phase-2 sub-items. The current branch is `v1/wsl2-avx512-par3-vs-par2` — an integration branch that holds mixed Phase 2a-c work, kernel work, and Phase 2b scaffolding across many sessions.
  - **Short-lived topic branches** (named `<type>/<scope>`, e.g. `feat/par3-cmdline-gap-analysis`, `fix-e2e-par3-repair`): receive a single focused PR's worth of work, merged and deleted.
  - Future Claude instances: identify which branch style is in use by `git branch --show-current`. Integration branches expect mid-stream commits; topic branches expect linear one-branch-one-PR discipline.
